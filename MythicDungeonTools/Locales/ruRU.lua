if not ((GAME_LOCALE or GetLocale()) == "ruRU") then
  return
end
local addonName, MDT = ...
local L = MDT.L
L = L or {}

-- MDT
L[" >Battle for Azeroth"] = ">Битва за Азерот"
L[" >Legion"] = ">Легион"
L[" K'aresh Elemental"] = "К'арешский элементаль"
L["%s HP"] = "%s Здоровье"
L["%s is in sublevel: %s"] = "%s находится на подуровне: %s"
L["(G %d)"] = "(Г %d)"
L["*Live*"] = "*Прямая трансляция*"
L["+ Add pull"] = "+ Добавить пулл"
L["<New Preset>"] = "<Новый маршрут>"
L["> More Dungeons"] = "> Больше подземелий"
L["4.RF-4.RF"] = "Г4В-Г4В"
L["Aberration"] = "Аберрация"
L["Abstract Nullifier"] = "Абстрактный нейтрализатор"
L["Abyss Dweller"] = "Обитатель глубин"
L["Abyssal Cultist"] = "Глубинный сектант"
L["Abyssal Eel"] = "Глубинный угорь"
L["Achillite"] = "Ахиллит"
L["Acidic Bile"] = "Едкая желчь"
L["Acolyte"] = "Послушник"
L["Adderis"] = "Гюрзис"
L["Addled Arcanomancer"] = "Одурманенный маг"
L["Addled Thug"] = "Одурманенный вышибала"
L["AddOn Conflict"] = "Конфликт дополнений"
L["Adorned Starseer"] = "Нарядный звездочет"
L["Advent Nevermore"] = "Адвент Невермор"
L["Advisor Melandrus"] = "Советник Меландр"
L["Aerial Unit R-21/X"] = "Воздушное судно R-21/X"
L["Affected by:"] = "Подвержен:"
L["Affixes"] = "Аффиксы"
L["Afflicted Civilian"] = "Пострадавший мирный житель"
L["Aggravated Skitterfly"] = "Рассерженная стрекотуха"
L["Agile Pursuer"] = "Проворный преследователь"
L["Agitated Nimbus"] = "Беспокойное облако"
L["Agronox"] = "Агронокс"
L["Ahn'kahar Slasher"] = "Ан'кахарский измельчитель"
L["Ahn'kahar Spell Flinger"] = "Ан'кахарский метатель заклинаний"
L["Ahn'kahar Swarmer"] = "Ан'кахарский паук"
L["Ahn'kahar Watcher"] = "Наблюдатель Ан'кахара"
L["Ahn'kahar Web Winder"] = "Ан'кахарский веретенщик"
L["Ahn'kahet: The Old Kingdom"] = "Ан'кахет: Старое Королевство"
L["Ahnzon"] = "Анзон"
L["Ahri'ok Dugru"] = "Ахри'ок Дугру"
L["Aka'ali the Conqueror"] = "Ака'али Завоевательница"
L["Alarm-o-Bot"] = "Тревогобот"
L["Alcruux"] = "Алькруукс"
L["Algeth'ar Echoknight"] = "Алгет'арский рыцарь эха"
L["AlgetharAcademy"] = "Академия Алгет'ар"
L["algetharShortName"] = "АА"
L["Alliance Archer"] = "Лучник Альянса"
L["Alliance Berserker"] = "Берсерк Альянса"
L["Alliance Cleric"] = "Священник Альянса"
L["Alliance Conjuror"] = "Кудесница из Альянса"
L["Alliance Destroyer"] = "Эсминец Альянса"
L["Alliance Footman"] = "Пехотинец Альянса"
L["Alliance Knight"] = "Рыцарь Альянса"
L["Alliance Ranger"] = "Следопыт Альянса"
L["Alpha Eagle"] = "Орел-вожак"
L["Altairus"] = "Альтаирий"
L["AltarOfShadow"] = "Альтарь тени"
L["altKeyDownStatusText"] = "Отображение номеров вражеских групп"
L["altKeyGroupsTip"] = "Удерживайте ALT, чтобы показать номера группы врагов на всех врагах"
L["Amalgam of Souls"] = "Слияние душ"
L["Amanitar"] = "Аманитар"
L["Amarth"] = "Амарт"
L["Ancient Captain"] = "Древний капитан"
L["Ancient Core Hound"] = "Древняя гончая недр"
L["Ancient Tome"] = "Древний фолиант"
L["Anduin Lothar"] = "Андуин Лотар"
L["Angerhoof Bull"] = "Сердитый бык"
L["Angry Hound"] = "Злой пес"
L["Anima Fountain"] = "Фонтан анимы"
L["Animated Droplet"] = "Оживленная капля"
L["Animated Guardian"] = "Оживший страж"
L["Animated Shadow"] = "Ожившая тень"
L["Animated Storm"] = "Ожившая буря"
L["Animated Weapon"] = "Оживленное оружие"
L["Anodized Coilbearer"] = "Анодированный разрядниконосец"
L["Anomalus"] = "Аномалус"
L["Anub'ar Champion"] = "Ануб'арский воитель"
L["Anub'ar Crusher"] = "Ануб'арский сокрушитель"
L["Anub'ar Crypt Fiend"] = "Ануб'арский некроарахнид"
L["Anub'ar Necromancer"] = "Ануб'арский некромант"
L["Anub'ar Prime Guard"] = "Ануб'арский стражник"
L["Anub'ar Shadowcaster"] = "Ануб'арский темный чародей"
L["Anub'ar Skirmisher"] = "Ануб'арский рыскатель"
L["Anub'ar Warrior"] = "Ануб'арский воин"
L["Anub'ar Webspinner"] = "Ануб'арский прядильщик"
L["Anub'arak"] = "Ануб'арак"
L["Anubikkaj"] = "Ануб'иккадж"
L["Anub'Rekhan"] = "Ануб'Рекан"
L["Anubzekt"] = "Ануб'зект"
L["Apex Blazewing"] = "Жарокрыл-вожак"
L["Apply to preset"] = "Применить к маршруту"
L["Aqua Rager"] = "Водяной яростень"
L["Aqu'sirr"] = "Аква'сирр"
L["Arachnopod Destroyer"] = "Арахнобот-разрушитель"
L["AraKara"] = "Ара-Кара, Город Отголосков"
L["araKaraItemDescription"] = "Может использоваться портными Каз Алгар (25)"
L["araKaraShortName"] = "АРА-КАРА"
L["Aranasi Broodmother"] = "Аранасийская мать выводка"
L["Arathi Footman"] = "Аратийский пехотинец"
L["Arathi Knight"] = "Аратийский рыцарь"
L["Arathi Neophyte"] = "Аратийский неофит"
L["Arcane Anomaly"] = "Волшебная аномалия"
L["Arcane Construct"] = "Чародейское создание"
L["Arcane Elemental"] = "Чародейский элементаль"
L["Arcane Eye"] = "Чародейское око"
L["Arcane Forager"] = "Чародейский добытчик"
L["Arcane Manifestation"] = "Проявление магии"
L["Arcane Minion"] = "Чародейский прислужник"
L["Arcane Ravager"] = "Чародейский опустошитель"
L["Arcane Siphon"] = "Вытягивание чародейской силы"
L["Arcane Tender"] = "Хранитель чар"
L["Arcane Warden"] = "Волшебный страж"
L["Arcanid"] = "Арканид"
L["Archdruid Glaidalis"] = "Верховный друид Глайдалис"
L["Archmage Sol"] = "Верховный маг Сол"
L["arcwayShortName"] = "КС"
L["Ardent Paladin"] = "Ревностный паладин"
L["Ardenweald"] = "Арденвельд"
L["Armored Mistral"] = "Бронированный мистраль"
L["Armored Overseer"] = "Тяжелый смотритель"
L["Armsmaster Harlan"] = "Воитель Гарлан"
L["Asaad"] = "Асаад"
L["Ascendant Viscoxria"] = "Вознесенный Вис'корксия"
L["Ascended Aristocrat"] = "Вознесенный аристократ"
L["Ash'Golm"] = "Вул'кан"
L["Ashvane Cannoneer"] = "Канонир дома Эшвейнов"
L["Ashvane Commander"] = "Командир из корпорации Эшвейнов"
L["Ashvane Deckhand"] = "Матрос корпорации Эшвейнов"
L["Ashvane Destroyer"] = "Разрушитель из дома Эшвейнов"
L["Ashvane Flamecaster"] = "Маг огня корпорации Эшвейнов"
L["Ashvane Invader"] = "Захватчик дома Эшвейнов"
L["Ashvane Jailer"] = "Тюремщик корпорации Эшвейнов"
L["Ashvane Marine"] = "Морпех корпорации Эшвейнов"
L["Ashvane Officer"] = "Офицер корпорации Эшвейнов"
L["Ashvane Priest"] = "Жрица корпорации Эшвейнов"
L["Ashvane Sniper"] = "Снайпер дома Эшвейнов"
L["Ashvane Spotter"] = "Наблюдатель корпорации Эшвейнов"
L["Ashvane Warden"] = "Стражник корпорации Эшвейнов"
L["Aspiring Forgehand"] = "Начинающий подмастерье"
L["Aspix"] = "Аспидис"
L["Asset Manager"] = "Управляющая активами"
L["assignmentWarning"] = "MDT НЕ будет автоматически устанавливать метки целей на врагах в игровом мире."
L["Assistant Unnaturaler"] = "Быстролап-помощник"
L["Astral Attendant"] = "Астральный служитель"
L["Astronos"] = "Астрон"
L["Atal'ai Deathwalker"] = "Вестник смерти из племени Атал'ай"
L["Atal'ai Devoted"] = "Верный служитель из племени Атал'ай"
L["Atal'ai High Priest"] = "Старший жрец из племени Атал'ай"
L["Atal'ai Hoodoo Hexxer"] = "Проклинатель худу из племени Атал'ай"
L["Atal'Dazar"] = "Атал'Дазар"
L["Atal'Dazar Sublevel"] = "Атал'Дазар"
L["atalDazarShortName"] = "АД"
L["atalTeemingNote"] =
  "При модификаторе 'Кишащий':%s%sГ29 не всегда присутствует.%sПротивники, добавленные модификатором 'Кишащий' в Г2, не всегда присутствуют.%sГ27 не всегда присутствует"
L["Athenaeum"] = "Библиотека"
L["Atik"] = "Атик"
L["Atrium of Sethraliss"] = "Атриум Сетралисс"
L["Attumen the Huntsman"] = "Ловчий Аттумен"
L["Auriaya"] = "Ауриайя"
L["Automatic Coloring"] = "Автоматическое окрашивание"
L["Automatically color pulls"] = "Автоматически окрашивать пуллы"
L["Avanoxx"] = "Аванокс"
L["Avatar of Sethraliss"] = "Аватара Сетралисс"
L["Awakened"] = "Пробудившийся"
L["Awakened A"] = "Пробудившийся (А)"
L["Awakened B"] = "Пробудившийся (Б)"
L["A'wazj"] = "А'вазж"
L["Azerite Extractor"] = "Экстрактор азерита"
L["Azerite Footbomb"] = "Азеритовая футбомба"
L["Azerokk"] = "Азерокк"
L["Azhiccar"] = "Ажиккар"
L["Azjol-Nerub"] = "Азжол-Неруб"
L["Azules"] = "Азуль"
L["Azure Captain"] = "Лазурный капитан"
L["Azure Enforcer"] = "Лазурный головорез"
L["Azure Inquisitor"] = "Лазурный инквизитор"
L["Azure Invader"] = "Лазурный захватчик"
L["Azure Ley-Whelp"] = "Детеныш лазурного дракона"
L["Azure Magus"] = "Лазурный волхв"
L["Azure Raider"] = "Лазурный налетчик"
L["Azure Scale-Binder"] = "Лазурный чешуеплет"
L["Azure Serpent"] = "Лазурный змей"
L["Azure Sorcerer"] = "Лазурная колдунья"
L["Azure Spellbinder"] = "Лазурный чароплет"
L["Azure Stalker"] = "Лазурный ловец"
L["Azure Warder"] = "Лазурный хранитель"
L["Azureblade"] = "Лазурный Клинок"
L["azureVaultShortName"] = "ЛХ"
L["B.O.O.M.B.A."] = "БУМБОТ"
L["Baalgar the Watchful"] = "Баалгар Бдительный"
L["Backup Singer"] = "Бэк-вокалистка"
L["Baelog"] = "Бейлог"
L["Balakar Khan"] = "Хан Балакар"
L["Balara"] = "Балара"
L["Band of Acceleration"] = "Кольцо ускорения"
L["Band of Alignment"] = "Кольцо управления"
L["Band of Transmutation"] = "Кольцо трансмутации"
L["Band of Variance"] = "Кольцо отклонения"
L["Banish"] = "Изгнание"
L["Banquet Steward"] = "Распорядитель банкета"
L["Barbed Spiderling"] = "Колючий паучок"
L["Baron Braunpyke"] = "Барон Браунпайк"
L["Baron Rivendare"] = "Барон Ривендер"
L["Batak"] = "Батак"
L["Battered"] = "Старый красный дракон"
L["Battle for Azeroth"] = "Битва за Азерот"
L["Battle Scarab"] = "Боевой скарабей"
L["Battlefield Ritualist"] = "Боевой ритуалист"
L["Bazaar Overseer"] = "Смотритель базара"
L["Bazaar Strongarm"] = "Базарный боец"
L["Beast"] = "Животное"
L["Bee Wrangler"] = "Укротитель пчел"
L["Bee-let"] = "Пчелка"
L["Befouled Spirit"] = "Оскверненный дух"
L["Beguiling"] = "Манящий"
L["Beguiling 1 Void"] = "Меченная Бездной посланница"
L["Beguiling 2 Tides"] = "Посланница глубин"
L["Beguiling 3 Ench."] = "Зачарованная посланница"
L["Benk Buzzbee"] = "Бенк Жужжикс <Пчеловод>"
L["Bewitched Captain"] = "Заколдованный капитан"
L["BFA"] = "Битва за Азерот"
L["Big M.O.M.M.A."] = "Большая МАМА"
L["Big Money Crab"] = "Краб-магнат"
L["Bile Golem"] = "Желчный голем"
L["Bile Retcher"] = "Желчерыг"
L["Bilespray Lasher"] = "Гнилостный плеточник"
L["Bilge Rat Brinescale"] = "Саурок из клана Соленой Чешуи"
L["Bilge Rat Buccaneer"] = "Буканьер из братства Трюмных Крыс"
L["Bilge Rat Cutthroat"] = "Головорез из братства Трюмных Крыс"
L["Bilge Rat Demolisher"] = "Крушитель из братства Трюмных Крыс"
L["Bilge Rat Looter"] = "Мародер из братства Трюмных Крыс"
L["Bilge Rat Padfoot"] = "Тихолап из братства Трюмных Крыс"
L["Bilge Rat Pillager"] = "Разоритель из братства Трюмных Крыс"
L["Bilge Rat Seaspeaker"] = "Морской колдун из братства Трюмных Крыс"
L["Bilge Rat Swabby"] = "Морячок из братства Трюмных Крыс"
L["Bilge Rat Tempest"] = "Воин бури из братства Трюмных Крыс"
L["Bilge Rats"] = "Трюмные крысы"
L["Binder Ashioi"] = "Подчинительница Ашиой"
L["Binding Javelin"] = "Сковывающее копье"
L["Bitterbrine Scavenger"] = "Мародер из племени Горькой Воды"
L["Bitterbrine Slave"] = "Раб из племени Горькой Воды"
L["Black and Yellow"] = "Черный и Желтый"
L["Black Blood"] = "Черная кровь"
L["Black Rook Hold"] = "Крепость Черной Ладьи"
L["blackrookHoldShortName"] = "КЧЛ"
L["Blacktar Bomber"] = "Метатель черной смолы"
L["Blacktooth"] = "Братство Чернозубых"
L["Blacktooth Arsonist"] = "Поджигатель из братства Чернозубых"
L["Blacktooth Brute"] = "Громила из братства Чернозубых"
L["Blacktooth Knuckleduster"] = "Кастетник из братства Чернозубых"
L["Blacktooth Scrapper"] = "Задира из братства Чернозубых"
L["Blade Dancer Illianna"] = "Иллиана Танцующая с Клинками"
L["Bladebeak Hatchling"] = "Птенец мечеклюва"
L["Bladebeak Matriarch"] = "Мечеклюв-матриарх"
L["Blastatron X-80"] = "Взрывотрон Х-80"
L["Blazebound Destroyer"] = "Пламенный разрушитель"
L["Blazebound Firestorm"] = "Шторм бушующего пламени"
L["Blazikon"] = "Пламекон"
L["Blazing Fiend"] = "Пылающий монстр"
L["Blazing Imp"] = "Пылающий бес"
L["Bleed"] = "Кровотечение"
L["Blight Bag"] = "Чумной мешок"
L["Blight Chunk"] = "Сгусток гнили"
L["Blight of Galakrond"] = "Гниль Галакронда"
L["Blight Toad"] = "Чумная жаба"
L["Blightbone"] = "Чумокост"
L["Blighted Sludge-Spewer"] = "Чумной изрыгатель слизи"
L["Blighted Spinebreaker"] = "Чумной спинолом"
L["Blightshard Shaper"] = "Заклинатель чумных осколков"
L["Blistering Steamrager"] = "Волдырный парозлоб"
L["Bloated Brew Alemental"] = "Раздувшийся бражный хмелементаль"
L["Block Warden"] = "Надзиратель блока"
L["Blood of the Corruptor"] = "Кровь Заразителя"
L["Blood Overseer"] = "Кровавый надсмотрщик"
L["Bloodscent Felhound"] = "Кровожадная гончая Скверны"
L["Bloodstained Assistant"] = "Окровавленный боец"
L["Bloodstained Webmage"] = "Окровавленный маг паутины"
L["Bloodsworn Agent"] = "Одержимый кровью шпион"
L["Bloodsworn Defiler"] = "Кровавый осквернитель"
L["Bloodtainted Fury"] = "Оскверненная кровью ярость"
L["Bloodthirsty Cub"] = "Кровожадный детеныш"
L["Bloodthirsty Tundra Wolf"] = "Кровожадный тундровый волк"
L["Bloodworker"] = "Кровавый служитель"
L["Bloody Javelin"] = "Кровавое копье"
L["Bolstering"] = "Усиливающий"
L["Bomb Tonk"] = "Бомботанк"
L["Bombshell Crab"] = "Краб-бомбопанцирник"
L["Bone Magus"] = "Костяной чародей"
L["Bonebolt Hunter"] = "Охотник-костестрел"
L["Bonegrinder"] = "Костомол"
L["Bonemaw"] = "Костебрюх"
L["Boneweaver"] = "Ткачиха Смерти"
L["Bony Construct"] = "Костлявое создание"
L["Boomer XP-500"] = "Бабахтор XP-500"
L["Bored Student"] = "Скучающий студент"
L["Borka the Brute"] = "Борка Бесчеловечный"
L["Bottom Feeder"] = "Донный краб"
L["Bound Air Elemental"] = "Укрощенный элементаль воздуха"
L["Bound Energy"] = "Обузданная энергия"
L["Bound Fire Elemental"] = "Укрощенный элементаль огня"
L["Bound Voidlord"] = "Подчиненный повелитель Бездны"
L["Bound Water Elemental"] = "Укрощенный элементаль воды"
L["Bounty Hunter"] = "Охотник за головами"
L[ [=[Bounty stacks
after this pull]=] ] = "После этого пулла награды стакаются"
L[ [=[Bounty stacks
after this pull]=] ] = "После этого пулла награда стакается"
L[ [=[Bounty stacks
after this pull]=] ] = "После этого пулла награда складывается"
L["Bracken Warscourge"] = "Бурая бичевательница"
L["Brackenhide Shaper"] = "Бурошкур-заклинательница"
L["brackenhideCage"] = "Склад мяса"
L["BrackenhideHollow"] = "Лощина Бурошкуров"
L["brackenhideShortName"] = "ЛБ"
L["Brain of Yogg-Saron"] = "Мозг Йогг-Сарона"
L["Brawling Patron"] = "Драчливый завсегдатай"
L["Brew Drop"] = "Хмельной выброс"
L["brewItemADescription"] =
  "Может использоваться гномами, механогномами, гоблинами или инженерами Каз Алгара (25)"
L["brewItemBDescription"] = "Может использоваться алхимиками Каз Алгара (25) или кулинарами Каз Алгара (25)"
L["Brewmaster Aldryr"] = "Хмелевар Алдрир"
L["Brittlebone Crossbowman"] = "Хрупкий арбалетчик"
L["Brittlebone Mage"] = "Хрупкий маг"
L["Brittlebone Warrior"] = "Хрупкий воин"
L["Bromach"] = "Бромач"
L["Bront"] = "Бронт"
L["Brood Ambusher"] = "Таящийся паук"
L["Brood Assassin"] = "Паук-убийца"
L["Brother Ironhull"] = "Брат Айронхалл"
L["Brother Korloff"] = "Брат Корлофф"
L["Brush Size"] = "Размер кисти"
L["Brutal Spire of Ny'alotha"] = "Жестокий шпиль Ни'алоты"
L["Bubbles"] = "Пузырек"
L["Bubbling Ooze"] = "Бурлящая жижа"
L["Burly Deckhand"] = "Дюжий матрос"
L["Burly Rock-Thrower"] = "Дюжий камнемет"
L["Burning Geode"] = "Пылающая жеода"
L["Burrowing Creeper"] = "Ползун-землерой"
L["Bursting"] = "Взрывной"
L["Cancel"] = "Отменить"
L["Candlestick Mage"] = "Маг с подсвечником"
L["CannonNote"] = "Тяжелое орудие%sИспользуется игроками%sНаносит урон как врагам, так и союзникам"
L["Cannot create preset '%s'"] = "Не удается создать маршрут '%s'"
L["Cannot export while in combat"] = "Невозможно экспортировать во время боя"
L["Cannot import while in combat"] = "Невозможно импортировать во время боя"
L["Cannot rename preset to '%s'"] = "Не удается переименовать маршрут в '%s'"
L["Captain Dailcry"] = "Капитан Дейлкрай"
L["Captain Eudora"] = "Капитан Юдора"
L["Captain Jolly"] = "Капитан Джолли"
L["Captain Raoul"] = "Капитан Рауль"
L["Careless Hopgoblin"] = "Неосторожный хмелегоблин"
L["Carrion Spinner"] = "Ткач-падальщик"
L["Carrion Worm"] = "Червь-трупоед"
L["Cartel Lackey"] = "Прислужник картеля"
L["Cartel Muscle"] = "Громила из картеля"
L["Cartel Skulker"] = "Тихоступ из картеля"
L["Cartel Smuggler"] = "Контрабандист из картеля"
L["Cartel Wiseguy"] = "Авторитет из картеля"
L["Catacombs"] = "Катакомбы"
L["Cathedral of Eternal Night"] = "Собор Вечной Ночи"
L["cathedralOfEternalNightShortName"] = "СВН"
L["Cavern Seeker"] = "Пещерный ловец"
L["Centrifuge Construct"] = "Центрифужное создание"
L["Chamber of Flames"] = "Чертог Пламени"
L["Chamber of Summoning"] = "Чертог Призыва"
L["Chamber Overseer"] = "Страж Вестибюля"
L["Chamber Sentinel"] = "Часовой чертогов"
L["ChamberOfFlames"] = "Чертог Пламени"
L["Champion Druna"] = "Защитница Друна"
L["Champion of Hodir"] = "Защитник Ходира"
L["Change Language"] = "Сменить язык"
L["Channeler Varisz"] = "Чародейка Варис"
L["Chaotic Rift"] = "Хаотический разлом"
L["Chargath, Bane of Scales"] = "Чаргат Дробитель Чешуи"
L["Charged Dust Devil"] = "Заряженная песчаная дьяволица"
L["chatNoninteractiveWarning"] =
  "Фрейм чата в настоящее время отключен от интерактивного режима, Вы не сможете нажимать на маршруты MDT."
L["Chef Chewie"] = "Шеф-повар Жевастик"
L["Choose Enemy Forces Format"] = "Выберите формат для отображения вражеских сил"
L["Choose Enemy Style. Requires Reload"] = "Выберите стиль для сил врага. Потребуется перезагрузка"
L["Choose NPC tooltip position"] = "Расположение подсказки для НИП"
L["Choose number of colors"] = "Выберите количество цветов"
L["Choose preferred color palette"] = "Выберите предпочтительную цветовую палитру"
L["Chopper Redhook"] = "Головорез Краснокрюк"
L["Chosen Blood Matron"] = "Избранная кровавая матрона"
L["Chromie"] = "Хроми"
L["Chronaxie"] = "Хронакси"
L["Chronikar"] = "Хроникар"
L["Chrono-Lord Deios"] = "Хронолорд Дейос"
L["Chrono-Lord Epoch"] = "Хронолорд Эпох"
L["Cinderbrew Meadery"] = "Искроварня"
L["cinderbrewShortName"] = "ИСКР"
L["CityofEchoes"] = "Город Отголосков"
L["CityOfThreads"] = "Город Нитей"
L["cityOfThreadsShortName"] = "ГН"
L["Claw Fighter"] = "Боец-коготь"
L["Clear all Markers"] = "Очистить все метки"
L["Click the fullscreen button for a maximized view of MDT."] =
  "Нажмите кнопку полноэкранного режима, чтобы развернуть MDT на весь экран."
L["Click to adjust color settings"] = "Нажмите, чтобы изменить настройки цвета"
L["Click to assign player"] = "Нажмите, чтобы назначить игрока"
L["Click to go to %s"] = "Нажмите, чтобы перейти к %s"
L["Click to set dungeon level to 10"] = "Нажмите, чтобы установить уровень подземелья на 10"
L["Click to switch to current week"] = "Нажмите, чтобы выбрать текущую неделю"
L["Click to toggle AddOn Window"] = "[ЛКМ] - открыть окно MDT"
L["Clicking this button will attempt to join the ongoing Live Session of your group or create a new one if none is found"] =
  "При нажатии этой кнопки будет произведена попытка присоединиться к 'Прямой трансляции' Вашей группы или создать новый, если таковой не найден"
L["Clockwork Sapper"] = "Робот-сапер"
L["Close"] = "Закрыть"
L["Cloud Prince"] = "Принц облаков"
L["Coalesced Moment"] = "Концентрированное мгновение"
L["Coalesced Time"] = "Сгусток времени"
L["Coalescing Void Diffuser"] = "Распылитель сгущающейся Бездны"
L["Coastwalker Goliath"] = "Береговой голиаф"
L["Coin-Operated Crowd Pummeler"] = "Платный разгонятель толпы"
L["Coldmist Stalker"] = "Паук-ловец холодного тумана"
L["Coldmist Widow"] = "Черная вдова холодного тумана"
L["Color Blind Friendly"] = "Режим для людей с дальтонизмом"
L["Colorpicker"] = "Палитра цветов"
L["Colossal Tentacle"] = "Огромное щупальце"
L["Combat Transparency"] = "Прозрачность во время боя"
L["Commander Durand"] = "Командир Дюранд"
L["Commander Kolurg"] = "Командир Колург"
L["Commander Lindon"] = "Командир Линдон"
L["Commander Ri'mok"] = "Командир Ри'мок"
L["Commander Shemdah'sohn"] = "Командир Шемда'сон"
L["Commander Stoutbeard"] = "Командир Пивобород"
L["Commander Ulthok"] = "Командир Улток"
L["Commander Vo'jak"] = "Командир Во'цзак"
L["Commander Zo'far"] = "Командир Зо'фар"
L["Commerce Enforcer"] = "Охранитель коммерции"
L["conflictPrompt"] =
  "MDT обнаружил, что у Вас установлены некоторые дополнения, конфликтующие с MDT. Удалите или обновите следующие дополнения:"
L["Congealed Droplet"] = "Сгустившаяся капля"
L["Congealed Slime"] = "Сгусток слизи"
L["Conjured Lasher"] = "Сотворенный плеточник"
L["ConnectedTip"] = "Группы, обведенные в MDT, не соответствуют объединению НИП в игре."
L["Consuming Sha"] = "Всепоглощающий ша"
L["Containment Apparatus"] = "Аппарат сдерживания"
L["Control Undead"] = "Порабощение нежити"
L["Cooking Pot"] = "Котелок"
L["copiedToClipboard"] = "скопировано!"
L["Copy"] = "Копировать"
L["Copy error"] = "Ошибка при копировании"
L["Cordana Felsong"] = "Кордана Оскверненная Песнь"
L["Corpse Collector"] = "Собиратель трупов"
L["Corpse Harvester"] = "Сборщик трупов"
L["Corpse Skitterling"] = "Трупный паучонок"
L["Corridor Creeper"] = "Коридорный ужас"
L["Corridor Sleeper"] = "Коридорный сновидец"
L["Corrupt Droplet"] = "Оскверненная капля"
L["Corrupt Living Water"] = "Оскверненная живая вода"
L["Corrupted Manafiend"] = "Оскверненный манадемон"
L["Corrupted Oracle"] = "Оскверненный оракул"
L["Corrupted Scroll"] = "Проклятый свиток"
L["Corrupted Servitor"] = "Зараженный служитель"
L["Corsair Brute"] = "Корсар-громила"
L["Corsair Officer"] = "Корсар-офицер"
L["Corstilax"] = "Корстилакс"
L["Court of Stars"] = "Квартал Звезд"
L["Court of Stars Sublevel"] = "Квартал Звезд"
L["courtOfStarsShortName"] = "КЗ"
L["Cove Seagull"] = "Чайка"
L["Coven Diviner"] = "Прорицательница из ковена"
L["Coven Thornshaper"] = "Заклинательница шипов из ковена"
L["Covert Webmancer"] = "Скрытый тенетомант"
L["Cragmaw the Infested"] = "Кроглот Зараженный"
L["Crawler Mine"] = "Ползучая мина"
L["Crawth"] = "Кроут"
L["Crazed Incubator"] = "Бешеная наседка"
L["Crazed Mana-Surge"] = "Безумный манаплеск"
L["Crazed Mana-Wraith"] = "Безумный призрак маны"
L["Crazed Marksman"] = "Обезумевший стрелок"
L["Crazed Razorbeak"] = "Обезумевший остроклюв"
L["Create"] = "Создать"
L["Create a new preset"] = "Создать новый маршрут"
L["Creepy Crawler"] = "Мерзкий ползун"
L["Critter"] = "Зверек"
L["Cruel Bonecrusher"] = "Жестокая костекрушительница"
L["Crusaders' Chapel"] = "Часовня Рыцаря"
L["Crypt Fiend"] = "Некроарахнид"
L["Crypt Reaver"] = "Некрорахнид-разоритель"
L["CryptOfTheAncients"] = "Склеп Древних"
L["Crystal Fury"] = "Кристальная ярость"
L["Crystal Shard"] = "Осколок кристалла"
L["Crystal Thrasher"] = "Кристальный крушитель"
L["CrystalChamber"] = "Кристальные чертоги"
L["Crystalline Frayer"] = "Кристаллический хлестун"
L["Crystalline Keeper"] = "Кристаллический хранитель"
L["Crystalline Protector"] = "Кристаллический заступник"
L["Crystalline Shardling"] = "Кристаллический осколыш"
L["Crystalline Tender"] = "Кристаллическая грация"
L["ctrlKeyCountTip"] = "Удерживайте CTRL, чтобы показать вражеские силы на всех врагах"
L["ctrlKeyDownStatusText"] = "Отображение вражеских сил"
L["Curious Swoglet"] = "Любопытный детеныш рогоплава"
L["Curse"] = "Проклятие"
L["Cursed Falke"] = "Проклятый сокол"
L["Cursed Rookguard"] = "Проклятый страж птенцов"
L["Cursed Rooktender"] = "Проклятый наставник буреклювов"
L["Cursed Spire of Ny'alotha"] = "Проклятый шпиль Ни'алоты"
L["Cursed Thunderer"] = "Проклятый рокотун"
L["Cursedforge Honor Guard"] = "Отважный страж проклятой кузни"
L["Cursedforge Mender"] = "Ремонтница проклятой кузни"
L["Cursedforge Stoneshaper"] = "Заклинатель проклятой кузни"
L["Cursedheart Invader"] = "Проклятый завоеватель"
L["Custom"] = "Пользовательский"
L["Custom Color Palette"] = "Пользовательская цветовая схема"
L["Customs Security"] = "Охранник таможни"
L["Cutwater"] = "Братство Волнорезов"
L["Cutwater Duelist"] = "Дуэлянт из братства Волнорезов"
L["Cutwater Harpooner"] = "Гарпунщик из братства Волнорезов"
L["Cutwater Knife Juggler"] = "Метатель ножей из братства Волнорезов"
L["Cutwater Striker"] = "Боец из братства Волнорезов"
L["Cyanigosa"] = "Синигоса"
L["Cyclone"] = "Смерч"
L["Cyclone Summit"] = "Вершина Смерча"
L["Dalronn the Controller"] = "Далронн Контролирующий"
L["Damaged Golem"] = "Поврежденный голем"
L["Dantalionax"] = "Данталионакс <Предатель надежды>"
L["Dargrul"] = "Даргрул"
L["Dark Acolyte"] = "Темная прислужница"
L["Dark Necromancer"] = "Темный некромант"
L["Dark Rune Controller"] = "Наблюдатель из клана Темных Рун"
L["Dark Rune Elementalist"] = "Повелитель стихий из клана Темных Рун"
L["Dark Rune Giant"] = "Великан клана Темных Рун"
L["Dark Rune Ravager"] = "Опустошитель из клана Темных Рун"
L["Dark Rune Scholar"] = "Ученый из клана Темных Рун"
L["Dark Rune Shaper"] = "Резчик из клана Темных Рун"
L["Dark Rune Theurgist"] = "Чудотворец из клана Темных Рун"
L["Dark Rune Thunderer"] = "Рокотун из клана Темных Рун"
L["Dark Rune Warrior"] = "Воин из клана Темных Рун"
L["Dark Rune Worker"] = "Рабочий из клана Темных Рун"
L["Dark Touched Warrior"] = "Помеченный Тьмой воин"
L["Darkflame Cleft"] = "Расселина Темного Пламени"
L["darkflameDespawns"] = "Неубитые враги в этой области исчезнут после события с вагонеткой"
L["darkflameShortName"] = "РТП"
L["darkflameSpawns"] = "Коридорный ужас и Снующая тьма появятся после события с вагонеткой"
L["Darkfuse Bloodwarper"] = "Кровоплет из картеля Мрачных Минеров"
L["Darkfuse Demolitionist"] = "Подрывник из картеля Мрачных Минеров"
L["Darkfuse Hyena"] = "Гиена картеля Мрачных Минеров"
L["Darkfuse Inspector"] = "Инспектор из картеля Мрачных Минеров"
L["Darkfuse Jumpstarter"] = "Запускатель из картеля Мрачных Минеров"
L["Darkfuse Mechadrone"] = "Мехадрон картеля Мрачных Минеров"
L["Darkfuse Soldier"] = "Солдат из картеля Мрачных Минеров"
L["Darkheart Thicket"] = "Чаща Темного Сердца"
L["Darkheart Thicket Sublevel"] = "Чаща Темного Сердца"
L["darkheartThicketShortName"] = "ЧТС"
L["Darkmaster Gandling"] = "Темный магистр Гандлинг"
L["Darkweb Hatchling"] = "Темнопутый детеныш"
L["Darkweb Recluse"] = "Темнопутый паук-отшельник"
L["DataImportButtonTooltip"] = "Импорт внешних данных NPC."
L["dawnBreakerShortName"] = "СР"
L["dawnlowerShortName"] = "РБПГ"
L["DawnOfTheInfiniteLower"] = "Рассвет Бесконечности: падение Галакронда"
L["DawnOfTheInfiniteUpper"] = "Рассвет Бесконечности: подъем Дорнозму"
L["dawnPortalNote"] =
  "Чтобы активировать порталы, необходимо победить Магов портала бесконечности 2 и 4. Временные искажения будут продолжать возрождаться, но не будут учитываться."
L["dawnupperShortName"] = "РБПД"
L["Dazar'ai Augur"] = "Дазар'айский авгур"
L["Dazar'ai Colossus"] = "Дазар'айский колосс"
L["Dazar'ai Confessor"] = "Дазар'айский духовник"
L["Dazar'ai Honor Guard"] = "Дазар'айский почетный страж"
L["Dazar'ai Juggernaut"] = "Дазар'айский сокрушитель"
L["Dazhak"] = "Дажак"
L["Dazzling Dragonfly"] = "Ослепительная стрекоза"
L["De Other Side"] = "Та Сторона"
L["Dealer Xy'exa"] = "Дилер Зи'екса"
L["Death Knight"] = "Рыцарь смерти"
L["Death Knight Captain"] = "Капитан рыцарей Смерти"
L["Death Knight Cavalier"] = "Кавалерист-рыцарь смерти"
L["Death Speaker"] = "Вестник смерти"
L["Deathcharger Steed"] = "Жеребец смерти"
L["Deathscreamer Ikentak"] = "Вестница гибели Икен'так"
L["Deathwalker"] = "Вестник смерти"
L["Decatriarch Wratheye"] = "Гниломаг Око Гнева"
L["Decay Speaker"] = "Голос разложения"
L["Decayed Elder"] = "Разложившийся старый древень"
L["Decaying Flesh Giant"] = "Разлагающийся великан"
L["Decaying Slime"] = "Разлагающийся слизень"
L["Decrease Brush Size"] = "Уменьшить размер кисти"
L["Deep Crawler"] = "Глубинный ползун"
L["Deep Murloc Drudge"] = "Глубинный мурлок - рабочий"
L["Deep Sea Murloc"] = "Глубоководный мурлок"
L["Deepsea Ritualist"] = "Глубоководный ритуалист"
L["Default"] = "По умолчанию"
L["defaultPresetName"] = "Маршрут"
L["Defective Sorter"] = "Дефективный сортировщик"
L["Defender of Many Eyes"] = "Защитник из многоокой рати"
L["Defense Bot Mk I"] = "Оборонобот I"
L["Defense Bot Mk III"] = "Оборонобот II"
L["Defier Draghar"] = "Драгхар Отрицатель"
L["Defiled Spire of Ny'alotha"] = "Оскверненный шпиль Ни'алоты"
L["Defiled Spirit"] = "Оскверненный дух"
L["Defunct Dental Drill"] = "Испорченная бормашина"
L["Delete"] = "Удалить"
L["Delete %s?"] = "Удалить %s?"
L["Delete ALL drawings"] = "Удалить ВСЕ нарисованные объекты"
L["Delete ALL presets"] = "Удалить ВСЕ маршруты"
L["Delete Preset"] = "Удалить маршрут"
L["Delete this preset"] = "Удалить этот маршрут"
L["deleteAllDrawingsPrompt"] =
  "Вы уверены, что хотите удалить ВСЕ рисунки из текущего маршрута?%sЭто нельзя отменить%s%s"
L["deleteAllWarning"] =
  "!!ВНИМАНИЕ!!%sВы действительно хотите удалить ВСЕ маршруты для этого подземелья?%sВы собираетесь удалить %s маршрут(а/ов)%sПосле удаления их нельзя будет восстановить"
L["Demolishing Terror"] = "Крушащий ужас"
L["Demolition Duo"] = "Пара подрывников"
L["Demon"] = "Демон"
L["DenOfDecay"] = "Гнилое логово"
L["deOtherSideShortName"] = "ТС"
L["Depleted Anima Seed"] = "Истощенное анимасемя"
L["depletedAnimaSeedDescription"] = "Обновляет местоположение кладбища"
L["Depraved Collector"] = "Порочная сборщица"
L["Depraved Darkblade"] = "Порочный темный клинок"
L["Depraved Houndmaster"] = "Порочный псарь"
L["Depraved Mistweaver"] = "Порочная ткачиха туманов"
L["Depraved Obliterator"] = "Порочный уничтожитель"
L["Depths Warden"] = "Страж глубин"
L["Desecrated Bakar"] = "Оскверненный бакар"
L["Desecrated Ohuna"] = "Оскверненный охун"
L["Despondent Scallywag"] = "Отчаявшийся негодяй"
L["Dessia the Decapitator"] = "Дессия Обезглавливательница"
L["Destroying Sha"] = "Разрушительный ша"
L["Detention Block"] = "Тюремный блок"
L["Detonating Crystal"] = "Детонирующий кристалл"
L["Devos"] = "Девия"
L["Devoted Accomplice"] = "Усердный подручный"
L["Devouring Ghoul"] = "Всепожирающий вурдалак"
L["Devouring Maggot"] = "Прожорливая личинка"
L["Devout Blood Priest"] = "Преданная жрица крови"
L["Devout Priest"] = "Благочестивый жрец"
L["Dimension Portal"] = "Пространственный портал"
L["Dinomancer Kish'o"] = "Диномант Киш'о"
L["Discarded Shield"] = "Брошенный щит"
L["Discharged Anima"] = "Освобожденная анима"
L["Disease"] = "Болезнь"
L["Disease Slasher"] = "Больная рубака"
L["Diseased Horror"] = "Зараженный ужас"
L["Diseased Lasher"] = "Больной плеточник"
L["Diseased Maggot"] = "Болезненная личинка-паразит"
L["Diseased Mastiff"] = "Зараженный мастиф"
L["Disgusting Refuse"] = "Отвратительные отходы"
L["Disorient"] = "Дизориентация"
L["Disruptive Patron"] = "Надоедливый завсегдатай"
L["Disturbed Kelp"] = "Потревоженные водоросли"
L["Dockhound Packmaster"] = "Портовый заводчик"
L["Doctor Ickus"] = "Доктор Икус"
L["Does not delete your drawings"] = "Не удаляет Ваши рисунки"
L["Dokigg the Brutalizer"] = "Докигг Изверг"
L["Domatrax"] = "Доматракс"
L["Domina Venomblade"] = "Домина Отравленный Клинок"
L["dotiLowerSublevel1"] = "Подуровень 1"
L["dotiUpperSublevel1"] = "Подуровень 1"
L["Draconic Illusion"] = "Драконья иллюзия"
L["Draconic Image"] = "Драконья иллюзия"
L["Drag the bottom right edge to resize MDT."] = "Потяните за нижний правый угол, чтобы изменить размер MDT."
L["Dragonflayer Ascent"] = "Подъем Укротителей драконов"
L["Dragonflayer Bonecrusher"] = "Костекрушитель из клана Укротителей драконов"
L["Dragonflayer Deathseeker"] = "Смертелюб из клана Укротителей драконов"
L["Dragonflayer Fanatic"] = "Фанатик из клана Укротителей драконов"
L["Dragonflayer Forge Master"] = "Выплавщик из клана Укротителей драконов"
L["Dragonflayer Heartsplitter"] = "Разрыватель сердец из клана Укротителей драконов"
L["Dragonflayer Ironhelm"] = "Железоголов из клана Укротителей драконов"
L["Dragonflayer Metalworker"] = "Металлорез из клана Укротителей драконов"
L["Dragonflayer Overseer"] = "Надзиратель из клана Укротителей драконов"
L["Dragonflayer Runecaster"] = "Заклинатель рун из клана Укротителей драконов"
L["Dragonflayer Seer"] = "Ясновидица клана Укротителей драконов"
L["Dragonflayer Spiritualist"] = "Спиритуалист из клана Укротителей драконов"
L["Dragonflayer Strategist"] = "Стратег из клана Укротителей драконов"
L["Dragonflayer Weaponsmith"] = "Оружейник из клана Укротителей драконов"
L["Dragonflight Season 1"] = "Драконы 1-й сезон"
L["Dragonflight Season 2"] = "Драконы 2-й сезон"
L["Dragonflight Season 3"] = "Драконы 3-й сезон"
L["Dragonflight Season 4"] = "Драконы 4-й сезон"
L["Dragonkin"] = "Драконид"
L["Drahga Shadowburner"] = "Драгх Горячий Мрак"
L["Drakkari Bat"] = "Нетопырь Драккари"
L["Drakkari Battle Rider"] = "Боевой наездник Драккари"
L["Drakkari Colossus"] = "Колосс Драккари"
L["Drakkari Commander"] = "Командир Драккари"
L["Drakkari Earthshaker"] = "Землетряс Драккари"
L["Drakkari Fire Weaver"] = "Огнепряд Драккари"
L["Drakkari Frenzy"] = "Бешенка Драккари"
L["Drakkari God Hunter"] = "Охотник бога Драккари"
L["Drakkari Golem"] = "Голем Драккари"
L["Drakkari Guardian"] = "Страж Драккари"
L["Drakkari Gutripper"] = "Потрошитель Драккари"
L["Drakkari Inciter"] = "Поджигатель Драккари"
L["Drakkari Lancer"] = "Пикейщик Драккари"
L["Drakkari Medicine Man"] = "Целитель Драккари"
L["Drakkari Raider"] = "Мародер Драккари"
L["Drakkari Rhino"] = "Люторог Драккари"
L["Drakkari Scytheclaw"] = "Смертехват Драккари"
L["Drakkari Shaman"] = "Шаман Драккари"
L["Drakonid Breaker"] = "Драконид-разрушительница"
L["Drakos the Interrogator"] = "Дракос Дознаватель"
L["Drak'Tharon Keep"] = "Крепость Драк'Тарон"
L["Drak'Tharon Overlook"] = "Дозорное укрепление Драк'Тарона"
L["Drawing: Arrow"] = "Рисование: стрелка"
L["Drawing: Eraser"] = "Рисование: ластик"
L["Drawing: Freehand"] = "Рисование: карандаш"
L["Drawing: Line"] = "Рисование: линия"
L["Dread Captain Lockwood"] = "Жуткий капитан Локвуд"
L["Dread Creeper"] = "Жуткий ползун"
L["Dreadborne Seer"] = "Жуткорожденный провидец"
L["Dreadfang"] = "Лютоклык"
L["Dreadfire Imp"] = "Бес ужасающего огня"
L["Dreadful Huntmaster"] = "Жуткий егерь"
L["Dreadhunter"] = "Жуткий охотник"
L["Dreadlord Mendacius"] = "Повелитель ужаса Мендаций"
L["Dreadpetal"] = "Страхоцвет"
L["Dreadsoul Poisoner"] = "Грозный отравитель"
L["Dreadsoul Ruiner"] = "Грозный разрушитель"
L["Dreadwing"] = "Грознокрыл"
L["Dreadwing Raven"] = "Грознокрылый ворон"
L["Dredged Sailor"] = "Утопший матрос"
L["Dresaron"] = "Дресарон"
L["Droman Oulfarran"] = "Дроман Ульфарран"
L["dropdownAssignPlayer"] = "Назначить игрока"
L["dropdownClear"] = "Очистить"
L["Drowned Depthbringer"] = "Утонувший вестник глубин"
L["Drums of War"] = "Барабаны войны"
L["Drunk Pirate"] = "Пьяный пират"
L["Drunken Hozen Brawler"] = "Пьяный хозен-буян"
L["Drust Boughbreaker"] = "Друст-древолом"
L["Drust Harvester"] = "Друст-жнец"
L["Drust Soulcleaver"] = "Друст-душеруб"
L["Drust Spiteclaw"] = "Друст - злобный коготь"
L["Dulhu"] = "Дулгу"
L["Dul'zak"] = "Дул'зак"
L["Dungeon Data Missing"] = "Данные о подземелье отсутствуют"
L["Dungeon Entrance"] = "Вход в подземелье"
L["Dungeon Level"] = "Уровень подземелья"
L["Duskwatch Arcanist"] = "Чародей из Сумеречной стражи"
L["Duskwatch Guard"] = "Караульный из Сумеречной стражи"
L["Duskwatch Reinforcement"] = "Резервист из Сумеречной стражи"
L["Duskwatch Sentry"] = "Часовой из Сумеречной стражи"
L["Dwarven Bomber"] = "Дворф-бомбист"
L["Dynamite Mine Cart"] = "Вагонетка с динамитом"
L["E.D.N.A"] = "ЗАЗУ"
L["Earlier Version"] =
  "У Вас уже есть маршрут с таким названием'%s'%sХотите внести изменения в него или создать новый?%s%s%s"
L["Earth Infused Golem"] = "Насыщенный землей голем"
L["Earthen Custodian"] = "Земельник-комендант"
L["Earthen Guardian"] = "Земельник-страж"
L["Earthen Warder"] = "Земельник-дозорный"
L["Earthen Weaver"] = "Ткач земли"
L["Earthrager"] = "Земляной яростень"
L["Earthshaper Telu"] = "Демиург Телу"
L["Ebonclaw Worg"] = "Эбеновый ворг"
L["Ebonstone Golem"] = "Голем из черного камня"
L["Echelon"] = "Эшелон"
L["Echo of Doragosa"] = "Эхо Дорагосы"
L["Eck the Ferocious"] = "Эк Свирепый"
L["Eco-Dome Al'dani"] = "Заповедник Аль'дани"
L["ecoDomeAldaniShortName"] = "ЗА"
L["eda2BossNote"] = "Для появления босса необходимо победить Г23"
L["EDAItem1Description"] = "Можно использовать |T4620672:15:15|t Наложение чар Каз Алгара (25)"
L["EdgeOfReality"] = "Грань реальности"
L["Edit"] = "Изменить"
L["Efficiency Score"] = "Показатель эффективности"
L["EfficiencyScoreTip"] =
  "Оценка эффективности означает, насколько ценно количество подземелий NPC по сравнению с их здоровьем."
L["Elaena Emberlanz"] = "Элейна Эмберланц"
L["Elder Brightleaf"] = "Древень Светлый Лист"
L["Elder Ironbranch"] = "Древень Железная Ветвь"
L["Elder Leaxa"] = "Старейшина Ликса"
L["Elder Nadox"] = "Старейшина Надокс"
L["Elder Shadeweaver"] = "Старейший заклинатель теней"
L["Elder Stonebark"] = "Древень Каменная Кора"
L["Elemental"] = "Элементаль"
L["Elfyra"] = "Эльфира"
L["Embalming Fluid"] = "Бальзамировочный состав"
L["Embalming Slime"] = "Бальзамирующая слизь"
L["Emberhusk Dominator"] = "Углепанцирный подчинитель"
L["Emberon"] = "Тлерон"
L["Embershard Scorpion"] = "Угольный скорпион"
L["Emissary of the Tides"] = "Посланница глубин"
L["Empty"] = "Пустой"
L["Empty Anima Vessel"] = "Пустой сосуд анимы"
L["Empyrean Assassin"] = "Небесный убийца"
L["Enable Compartment Button"] = "Отображать иконку MDT в 'Отделении для аддонов'"
L["Enable Minimap Button"] = "Показывать иконку MDT на миникарте"
L["Enchanted Broodling"] = "Зачарованный паучонок"
L["Enchanted Emissary"] = "Зачарованная посланница"
L["encounteredErrors"] = "MDT обнаружил ошибки. Нажмите, чтобы просмотреть их."
L["Encrypted"] = "Зашифрованный"
L["Enemies related to seasonal affixes are currently hidden"] =
  "Враги, связанные с сезонными аффиксами, в настоящее время скрыты"
L["Enemy Info"] = "Информация о противнике"
L["Enemy Info NPC Creature Type"] = "Тип существа"
L["Enemy Info NPC Enemy Forces"] = "Силы врага"
L["Enemy Info NPC Enemy Forces (Teeming)"] = "Силы врага (Кишащий)"
L["Enemy Info NPC Health"] = "Здоровье (+%d %s)"
L["Enemy Info NPC Id"] = "Идентификатор НИП"
L["Enemy Info NPC Level"] = "Уровень"
L["Enemy Info NPC Name"] = "Имя"
L["Enemy Info NPC Stealth"] = "Незаметность"
L["Enemy Info NPC Stealth Detect"] = "Обнаружение незаметности"
L["Engine Speaker"] = "Глашатай машины"
L["Engorged Crawler"] = "Разъевшийся ползун"
L["Enrage"] = "Исступление"
L["Enraged Direhorn"] = "Разъяренный дикорог"
L["Enraged Mask"] = "Яростная маска"
L["Enraged Spirit"] = "Разъяренный дух"
L["Enraging Ghoul"] = "Разъярившийся вурдалак"
L["Enslave Demon"] = "Порабощение демона"
L["Enslaved Proto-Drake"] = "Порабощенный протодракон"
L["Enslaved Shieldmaiden"] = "Порабощенная дева щита"
L["Enthralled Guard"] = "Порабощенный стражник"
L["Entropic Spire of Ny'alotha"] = "Энтропический шпиль Ни'алоты"
L["Epoch Ripper"] = "Разрыватель эпох"
L["Eredar Chaosbringer"] = "Эредарский вестник хаоса"
L["Erekem"] = "Эрекем"
L['Eric "The Swift"'] = "Эрик Быстрый"
L["Erkhart Stormvein"] = "Эркхарт Кровь Бури"
L["Error Message:"] = "Сообщение об ошибке:"
L["errorLabel1"] = "MDT столкнулся с ошибками"
L["errorLabel2"] =
  "Пожалуйста, обновите MDT до последней версии, посетите GitHub или Discord и сообщите об ошибке ниже."
L["errorLabel3"] = "Нажмите CTRL+C, чтобы скопировать!"
L["Erudax"] = "Эрудакс"
L["Erudite Slayer"] = "Убийца эрудитов"
L["Erunak Stonespeaker"] = "Эрунак Говорящий с Камнем"
L["Etherdiver"] = "Эфирный Ныряльщик"
L["Ethereal Restorer"] = "Алгет'арский целитель"
L["Ethereal Sha"] = "Призрачный ша"
L["Everbloom Cultivator"] = "Культиватор Вечного Цветения"
L["Everbloom Mender"] = "Лекарь Вечного Цветения"
L["Everbloom Naturalist"] = "Натуралист Вечного Цветения"
L["everbloomShortName"] = "ВЦ"
L["Executioner Varruth"] = "Палач Варрут"
L["Executor of the Caliph"] = "Палач калифа"
L["Executor Tarvold"] = "Вершитель Тарвольд"
L["Exhumed Spirit"] = "Эксгумированный дух"
L["Expand the top toolbar to gain access to drawing and note features."] =
  "Разверните боковую панель инструментов, чтобы получить доступ к функциям рисования и заметок."
L["Experimental Sludge"] = "Экспериментальная жижа"
L["Expert Technician"] = "Техник-эксперт"
L["Explosive"] = "Взрывоопасный"
L["Export"] = "Экспорт"
L["Export the preset as a text string"] = "Экспорт текущего маршрута в виде текстовой строки"
L["ExportStringShareExternalWebsite"] = "Вы можете поделиться экспортом MDT на внешних сайтах"
L["Eye of Azshara"] = "Око Азшары"
L["Eye of Azshara Sublevel"] = "Око Азшары"
L["Eye of Sethraliss"] = "Глаз Сетралисс"
L["Eye of Taldaram"] = "Глаз Талдарама"
L["Eye Of The Queen"] = "Око королевы"
L["Eye Stalk"] = "Глазной стебелек"
L["eyeOfAzsharaShortName"] = "ОА"
L["Faceless Corruptor"] = "Безликий осквернитель"
L["Faceless Horror"] = "Безликий ужас"
L["Faceless Maiden"] = "Безликая дева"
L["Faceless Seer"] = "Безликий провидец"
L["Faceless Watcher"] = "Безликий дозорный"
L["Faithless Tender"] = "Отступник - хранитель яиц"
L["Fallen Deathspeaker"] = "Падший вестник смерти"
L["Fallen Waterspeaker"] = "Падший заклинатель воды"
L["Famished Broken"] = "Изголодавшийся сломленный"
L["Famished Tick"] = "Изголодавшийся клещ"
L["Fanatical Conjuror"] = "Окудница-фанатичка"
L["Fanatical Headhunter"] = "Фанатичный охотник за головами"
L["Fear"] = "Страх"
L["Feasting Skyscreamer"] = "Ненасытный небесный крикун"
L["Feckless Assistant"] = "Ленивый лаборант"
L["Fel Bat"] = "Сквернотопырь"
L["Fel Scorcher"] = "Выжигатель Скверны"
L["Felblight Stalker"] = "Охотник гибельной Скверны"
L["Felborne Botanist"] = "Присягнувшая Скверне - ботаник"
L["Felbound Enforcer"] = "Порабощенная Скверной карательница"
L["Felguard Destroyer"] = "Страж Скверны - разрушитель"
L["Fel-Infused Fury"] = "Зараженный Скверной яростный боец"
L["Felspite Dominator"] = "Злобный покоритель Скверны"
L["Felstrider Enforcer"] = "Сквернолап-каратель"
L["Felstrider Orbcaster"] = "Сквернолап - метательница сфер"
L["Felsworn Infester"] = "Скверноподданный заразитель"
L["Felsworn Myrmidon"] = "Скверноподданный мирмидон"
L["Fen Hatchling"] = "Маленький болотный шершень"
L["Fen Hornet"] = "Болотный шершень"
L["Fenryr"] = "Фенрир"
L["Feral Bloodswarmer"] = "Дикий кровавый роевик"
L["Fervent Sharpshooter"] = "Яростный снайпер"
L["Festerhide Grizzly"] = "Гноешкурый гризли"
L["Fetid Maggot"] = "Смрадная личинка"
L["Fetid Rotsinger"] = "Зловонная певица гнили"
L["Feugen"] = "Фойген"
L["ffAbandonTip"] = "Используйте команду /ff для начала голосования по отказу от M+."
L["Field of the Eternal Hunt"] = "Поля вечной охоты"
L["Filth Caller"] = "Призывательница гнили"
L["Finger Food"] = "Пикантные пальчики"
L["Fishface"] = "Рыбоморд"
L["Flame Channeler"] = "Чаротворец пламени"
L["Flame Leviathan"] = "Огненный Левиафан"
L["Flamecaller Aymi"] = "Призывательница пламени Аюми"
L["Flamegullet"] = "Огнезев"
L["Flamethrower"] = "Огнемет"
L["Flameweaver Koegler"] = "Ткач пламени Кеглер"
L["Flashfrost Chillweaver"] = "Ткач Холода Морозной Вспышки"
L["Flashfrost Earthshaper"] = "Демиург Морозной Вспышки"
L["Flavor Scientist"] = "Исследовательница вкусов"
L["Fleeting Manifestation"] = "Изменчивое воплощение"
L["Flesh Crafter"] = "Изготовитель кадавров"
L["Flesh Horror"] = "Ужас во плоти"
L["Flesheating Ghoul"] = "Плотоядный вурдалак"
L["Fleshrender Nok'gar"] = "Раздиратель Плоти Нок'Гар"
L["Fleshripper Vulture"] = "Стервятник-шкуродер"
L["Flying Snow"] = "Летящий Снег"
L["Focused Ritualist"] = "Настойчивый ритуалист"
L["Font of Fealty"] = "Источник Преданности"
L["Footbomb Hooligan"] = "Футбомбный хулиган"
L["Forces"] = "Силы врага"
L["Forces only: 5/200"] = "Только силы врага: 5/200"
L["Forces+%: 5/200 (2.5%)"] = "Силы врага+%: 5/200 (2.5%)"
L["Forest Swarmer"] = "Лесной плеточник"
L["Forge Construct"] = "Порождение горна"
L["Forge Loader"] = "Кузнечный погрузчик"
L["Forge Master Damian"] = "Мастер кузни Дамиан"
L["Forgebound Mender"] = "Кованая целительница"
L["Forgemaster Gorek"] = "Главный кузнец Горек"
L["Forgemaster Throngus"] = "Начальник кузни Тронг"
L["Forgewrought Monstrosity"] = "Выкованное чудовище"
L["Forgotten Denizen"] = "Забытый обитатель глубин"
L["Forgotten One"] = "Позабытый"
L["Forgotten Spirit"] = "Забытая душа"
L["Forlorn Cloister"] = "Покинутый двор"
L["Forlorn Spirit"] = "Неприкаянный дух"
L["Forsworn Castigator"] = "Раскольница-бичевательница"
L["Forsworn Champion"] = "Раскольник-защитник"
L["Forsworn Goliath"] = "Голиаф раскольников"
L["Forsworn Helion"] = "Раскольник-бунтарь"
L["Forsworn Inquisitor"] = "Раскольница-инквизитор"
L["Forsworn Justicar"] = "Раскольница-юстициарий"
L["Forsworn Mender"] = "Раскольник-целитель"
L["Forsworn Skirmisher"] = "Раскольник-налетчик"
L["Forsworn Squad-Leader"] = "Раскольник-офицер"
L["Forsworn Stealthclaw"] = "Стальной коготь раскольников"
L["Forsworn Usurper"] = "Раскольник-узурпатор"
L["Forsworn Vanguard"] = "Раскольник - боец авангарда"
L["Forsworn Warden"] = "Раскольник-хранитель"
L["Fortified"] = "Укрепленный"
L["Forward Train Cars"] = "Передние вагоны"
L["Foul Mother"] = "Темная мать"
L["frackingNote"] =
  "Фрекинговый тотем%sИспользуется игроками%sПарализует Земляного яростня на 1 минуту - рассеивается при получении урона"
L["Fragment of Hatred"] = "Фрагмент Ненависти"
L["Fragrant Lotus"] = "Благоухающий Лотос"
L["Freehold"] = "Вольная Гавань"
L["Freehold Barhand"] = "Официантка из Вольной Гавани"
L["Freehold Deckhand"] = "Палубный матрос из Вольной Гавани"
L["Freehold Pack Mule"] = "Вьючный мул из Вольной Гавани"
L["Freehold Shipmate"] = "Моряк из Вольной Гавани"
L["Freehold Sublevel"] = "Вольная Гавань"
L["freeholdBeguilingPatrolNote"] = "Неделя 2/5/8/11: Г53 неподвижен при Посланнице глубин 13"
L["freeholdGraveyardDescription1"] = "Доступно после победы над Небесным капитаном Краггом"
L["freeholdGraveyardDescription2"] = "Доступно после победы над Советом капитанов"
L["freeholdShortName"] = "ВГ"
L["Frenzied Bat"] = "Бесноватый нетопырь"
L["Frenzied Geist"] = "Бесноватый упырь"
L["Frenzied Ghoul"] = "Взбесившийся вурдалак"
L["Frenzied Mite"] = "Бешеный клещ"
L["Frenzied Nightclaw"] = "Разъяренный коготь ночи"
L["Frenzied Spirit"] = "Бесноватый дух"
L["Freya"] = "Фрейя"
L["Frostbringer"] = "Стужень"
L["Fulminating Lasher"] = "Гремучий плеточник"
L["Fungalmancer"] = "Грибомаг"
L["Fungi Stormer"] = "Гриб-штурмовик"
L["Fungret Shroomtender"] = "Мицелляр-культиватор"
L["G.U.A.R.D."] = "СТРАЖ"
L["Gal'darah"] = "Гал'дара"
L["Galecaller Apprentice"] = "Ученик призывательницы штормов"
L["Galecaller Faye"] = "Призывательница штормов Фэй"
L["Galvazzt"] = "Гальваззт"
L["Gamesman's Hall"] = "Игровой зал"
L["Gardens of Repose"] = "Сады Отдохновения"
L["Gashtooth"] = "Рви-зуб"
L["Gate of the Setting Sun"] = "Врата Заходящего Солнца"
L["Gate Watch Tower"] = "Дозорная башня"
L["gateOfTheSettingSunShortName"] = "ВЗС"
L["Gatewarden Zo'mazz"] = "Страж ворот Зо'мазз"
L["Gazerax"] = "Созерцатель"
L["Geezle Gigazap"] = "Гизл Гигабжик"
L["Gekkan"] = "Геккан"
L["General Bjarngrim"] = "Генерал Бьярнгрим"
L["General Kaal"] = "Генерал Кааль"
L["General Pa'valak"] = "Генерал Па'валак"
L["General Umbriss"] = "Генерал Умбрисс"
L["General Vezax"] = "Генерал Везакс"
L["General Xakal"] = "Генерал Ксакал"
L["Gerenth the Vile"] = "Герент Зловещий"
L["Ghastly Parishioner"] = "Жуткий молельщик"
L["Ghastly Voidsoul"] = "Жуткая душа из Бездны"
L["Ghostly Baker"] = "Бестелесный пекарь"
L["Ghostly Chef"] = "Бестелесный повар"
L["Ghostly Councilor"] = "Фантомный советник"
L["Ghostly Philanthropist"] = "Бестелесный филантроп"
L["Ghostly Protector"] = "Фантомный защитник"
L["Ghostly Retainer"] = "Фантомный вельможа"
L["Ghostly Steward"] = "Бестелесный распорядитель"
L["Ghostly Understudy"] = "Бестелесный дублер"
L["Ghoul Tormentor"] = "Вурдалак-мучитель"
L["Giant"] = "Великан"
L["Gilded Priestess"] = "Позолоченная жрица"
L["Gildedfur Stag"] = "Золотистый олень"
L["Gilgoblin Aquamage"] = "Жаблин - водный маг"
L["Gilgoblin Hunter"] = "Жаблин-охотник"
L["Glacial Proto-Dragon"] = "Ледниковый протодракон"
L["Glayvianna Soulrender"] = "Терзательница душ Глевианна"
L["Glazer"] = "Смотрящий"
L["Glintrok Greenhorn"] = "Глинтрок-зеленорог"
L["Glintrok Hexxer"] = "Глинтрок - мастер проклятий"
L["Glintrok Ironhide"] = "Глинтрок-крепкошкур"
L["Glintrok Oracle"] = "Глинтрок-оракул"
L["Glintrok Scout"] = "Глинтрок-разведчик"
L["Glintrok Skulker"] = "Глинтрок-тихоступ"
L["Globgrog"] = "Склизгл"
L["Gloom Horror"] = "Сумрачный ужас"
L["Gluth"] = "Глут"
L["Gluttonous Tick"] = "Ненасытный клещ"
L["Gnarled Ancient"] = "Крючковатый древний"
L["Gnarlroot"] = "Кривокорень"
L["Gnome-Eating Slime"] = "Слизнюк-гномоед"
L["Gnomercy 4.U."] = "Гномогедд-0Н"
L["God-King Skovald"] = "Король-бог Сковальд"
L["Goldie Baronbottom"] = "Голди Барондон <Надзирательница медоварни>"
L["Gorak Tul"] = "Горак Тул"
L["Gorechop"] = "Кроворуб"
L["Goregrind"] = "Кровомес"
L["Goregrind Bits"] = "Куски Кровомеса"
L["Gorestained Piglet"] = "Запачканный кровью поросенок"
L["Gortok Palehoof"] = "Горток Бледное Копыто"
L["Gothik the Harvester"] = "Готик Жнец"
L["Grain Cellar"] = "Зерновой амбар"
L["Grand Magus Telestra"] = "Великая ведунья Телестра"
L["Grand Overseer"] = "Старший смотритель"
L["Grand Proctor Beryllia"] = "Верховная надзирательница Бериллия"
L["Grand Shadow-Weaver"] = "Великий заклинатель теней"
L["Grand Vizier Ertan"] = "Великий визирь Эртан"
L["Grand Widow Faerlina"] = "Великая вдова Фарлина"
L["Granyth"] = "Гранит"
L["Graveyard"] = "Кладбище"
L["Grease Bot"] = "Смазочный бот"
L["Grievous"] = "Мучительный"
L["Grim Batol"] = "Грим Батол"
L["grimBatolShortName"] = "ГБ"
L["Grimhorn the Enslaver"] = "Злобнорог Поработитель"
L["Grimrail Bombardier"] = "Бомбардир Мрачных Путей"
L["Grimrail Laborer"] = "Рабочий Мрачных Путей"
L["Grimrail Loader"] = "Заряжающий Мрачных Путей"
L["Grimrail Overseer"] = "Надзиратель Мрачных Путей"
L["Grimrail Scout"] = "Разведчица Мрачных Путей"
L["Grimrail Technician"] = "Техник Мрачных Путей"
L["GrimrailDepot"] = "Депо Мрачных Путей"
L["grimrailDepotShortName"] = "ДМП"
L["Grip"] = "Хватка смерти"
L["Gripping Terror"] = "Цепкий ужас"
L["Gritslime Snail"] = "Песчаная улитка"
L["Grobbulus"] = "Гроббулус"
L["Grom'kar Battlemaster"] = "Гром'карский военачальник"
L["Grom'kar Boomer"] = "Гром'карский подрывник"
L["Grom'kar Captain"] = "Гром'карский капитан"
L["Grom'kar Chainmaster"] = "Гром'карский мастер цепей"
L["Grom'kar Cinderseer"] = "Гром'карская пророчица на пепле"
L["Grom'kar Deadeye"] = "Гром'карский остроглаз"
L["Grom'kar Deckhand"] = "Гром'карский матрос"
L["Grom'kar Far Seer"] = "Гром'карский ясновидящий"
L["Grom'kar Flameslinger"] = "Гром'карская огненная лучница"
L["Grom'kar Footsoldier"] = "Гром'карский пехотинец"
L["Grom'kar Grenadier"] = "Гром'карский гренадер"
L["Grom'kar Gunner"] = "Гром'карская опалительница"
L["Grom'kar Hulk"] = "Гром'карский исполин"
L["Grom'kar Incinerator"] = "Гром'карский поджигатель"
L["Grom'kar Technician"] = "Гром'карский техник"
L["Grotesque Horror"] = "Искаженный ужас"
L["Grubby Dirtcruncher"] = "Чумазый гряземес"
L["Gu Cloudstrike"] = "Гу Небесный Удар"
L["Guard Captain Atu"] = "Капитан стражи Ату"
L["Guard Captain Suleyman"] = "Капитан стражи Сулейман"
L["Guardian Construct"] = "Голем-страж"
L["Guardian Elemental"] = "Сторожевой элементаль"
L["Guardian Lasher"] = "Страж-плеточник"
L["Guardian of Life"] = "Хранитель жизни"
L["Guardian Sentry"] = "Охранник"
L["Guardian's Library"] = "Библиотека хранителя"
L["Gulping Goliath"] = "Голиаф-проглот"
L["Gundrak"] = "Гундрак"
L["Gunker"] = "Токсикоид"
L["Gurthan Iron Maw"] = "Стальная пасть клана Гуртан"
L["Gurthan Swiftblade"] = "Молниеносный клинок клана Гуртан"
L["Gushing Slime"] = "Хлещущая слизь"
L["Gust Soldier"] = "Клубящийся солдат"
L["Gusting Proto-Dragon"] = "Воздушный протодракон"
L["Gutchewer Bear"] = "Медведь-кишкожуй"
L["Gutshot"] = "Пузострелка"
L["Gutstabber"] = "Брюхокол"
L["Hadal Darkfathom"] = "Хадал Черная Бездна"
L["Hadronox"] = "Хадронокс"
L["Hadronox's Lair"] = "Логово Хадронокса"
L["Haiyan the Unstoppable"] = "Хайан Неудержимый"
L["Hakkar the Soulflayer"] = "Хаккар Свежеватель Душ"
L["Halkias"] = "Халкиас"
L["Hall of the Keepers"] = "Зал Хранителей"
L["Hall of the Moon"] = "Зал Луны"
L["HallOfTheKeepers"] = "Зал Хранителей"
L["Halls of Atonement"] = "Чертоги Покаяния"
L["Halls of Infusion"] = "Чертоги Насыщения"
L["Halls of Lightning"] = "Чертоги Молний"
L["Halls of Stone"] = "Чертоги Камня"
L["Halls of Valor"] = "Чертоги Доблести"
L["Halls of Valor Sublevel"] = "Чертоги Доблести"
L["HallsOfAtonementFloor1"] = "Чертоги Покаяния"
L["HallsOfAtonementFloor2"] = "Неф Боли"
L["HallsOfAtonementFloor3"] = "Святилище Душ"
L["hallsOfAtonementShortName"] = "ЧП"
L["HallsOfInfusion"] = "Чертоги насыщения"
L["hallsOfInfusionShortName"] = "ЧН"
L["hallsOfValorShortName"] = "ЧД"
L["Hapless Assistant"] = "Незадачливый ассистент"
L["Harbaron"] = "Харбарон"
L["Hardened Iron Golem"] = "Голем из закаленного железа"
L["Hardened Steel Berserker"] = "Берсерк из закаленной стали"
L["Hardened Steel Reaver"] = "Разоритель из закаленной стали"
L["Hardened Steel Skycaller"] = "Небоход из закаленной стали"
L["hardResetButton"] = "Сброс MDT"
L["hardResetPrompt"] =
  "Вы действительно хотите сбросить все настройки по умолчанию?\\n|cFFFF0000Попробуйте сначала обновить дополнение до последней версии!\\nДелайте это только в том случае, если MDT вообще не загружается и находится в неисправном состоянии.\\nЭто приведет к удалению ВСЕХ ВАШИХ МАРШРУТОВ! |r\\nЭто перезагрузит пользовательский интерфейс!"
L["hardResetPromptTitle"] = "Удалить ВСЕ настройки"
L["Harlan Sweete"] = "Красавчик Харлан"
L["Harthak Flameseeker"] = "Хартак Поджигатель"
L["Harthak Stormcaller"] = "Призыватель бури из клана Хартак"
L["Harugia the Bloodthirsty"] = "Харугия Кровожадная"
L["Hatchling Nest"] = "Чумное гнездо"
L["Hatecoil Arcanist"] = "Колдунья из клана Колец Ненависти"
L["Hatecoil Crusher"] = "Мирмидон из клана Колец Ненависти"
L["Hatecoil Oracle"] = "Оракул из клана Колец Ненависти"
L["Hatecoil Stormweaver"] = "Заклинательница штормов из клана Колец Ненависти"
L["Hatecoil Warrior"] = "Воин из клана Колец Ненависти"
L["Hatecoil Wavebinder"] = "Покорительница волн из клана Колец Ненависти"
L["Hatecoil Wrangler"] = "Ловчий из клана Колец Ненависти"
L["Hateful Essence"] = "Злобная сущность"
L["Hatespawn Slime"] = "Порожденный ненавистью слизень"
L["Haunting Sha"] = "Навязчивый ша"
L["Head Custodian Javlin"] = "Главный смотритель Джавлин"
L["Head Machinist Sparkflux"] = "Главный машинист Искроточец"
L["Headless Client"] = "Безголовый клиент"
L["Headmaster's Study"] = "Кабинет ректора"
L["Heart Guardian"] = "Хранитель Сердца"
L["Heartsbane Runeweaver"] = "Плетельщица рун из ковена Мертвых Сердец"
L["Heartsbane Soulcharmer"] = "Исказительница душ из ковена Мертвых Сердец"
L["Heartsbane Vinetwister"] = "Прядильщица лоз из ковена Мертвых Сердец"
L["Heavin the Breaker"] = "Хивин Ломатель"
L["Heavy Scrapbot"] = "Тяжелый хламобот"
L["Heigan the Unclean"] = "Хейган Нечестивый"
L["Helarjar Champion"] = "Хеларьяр-защитник"
L["Helarjar Mistcaller"] = "Хеларьяр - призывательница туманов"
L["Helblaze Felbringer"] = "Вестник Скверны адского огня"
L["Helblaze Imp"] = "Бес адского огня"
L["Helblaze Soulmender"] = "Врачеватель душ адского огня"
L["Hellblaze Temptress"] = "Искусительница адского огня"
L["Helmouth Cliffs"] = "Утесы Адской Пасти"
L["helpPlateDungeon"] = "Пользовательская настройка опций подземелья"
L["helpPlateDungeonSelect"] = "Выберите подземелье и перейдите на разные подуровни"
L["helpPlateNPC"] =
  "Щелкните, чтобы выбрать врагов%s[CTRL+ЛКМ] - выбрать одного врага%s[SHIFT+ЛКМ] - выбрать врагов и создать новый пулл"
L["helpPlatePresets"] = "Придумывайте маршруты, управляйте и делитесь ими"
L["helpPlatePulls"] = "Создавайте пуллы и управляйте ими%s[ПКМ] - открыть дополнительные параметры"
L["Helya"] = "Хелия"
L["Herald Of Ansurek"] = "Глашатай Ансурек"
L["Herald Volazj"] = "Глашатай Волаж"
L["Hibernate"] = "Спячка"
L["High Adjudicator Aleez"] = "Верховный адъюдикатор Ализа"
L["High Channeler Ryvati"] = "Верховная чаротворица Ривати"
L["High Contrast"] = "Высокая контрастность"
L["High Inquisitor Whitemane"] = "Верховный инквизитор Вайтмейн"
L["High Priest Aemya"] = "Верховная жрица Эмия"
L["High Speaker Eirich"] = "Верховный глашатай Эйрих"
L["Hired Assassin"] = "Наемный убийца"
L["Hired Muscle"] = "Бугай"
L["HK-8 Aerial Oppression Unit"] = "Воздушный подавитель ОУ-8"
L["Hodir"] = "Ходир"
L["Hold CTRL to single-select enemies."] = "Удерживайте CTRL, чтобы выбирать противников по одному."
L["Hold SHIFT to create a new pull while selecting enemies."] =
  "Удерживайте SHIFT, чтобы создавать отдельные пуллы при выборе врагов"
L["Hold SHIFT to delete all presets with the delete preset button."] =
  'Удерживайте SHIFT, чтобы удалить все маршруты при помощи кнопки "удалить маршрут".'
L["Honored Raptor"] = "Почитаемый ящер"
L["Honor's Ascent"] = "Подъем Чести"
L["Hoodoo Hexer"] = "Проклинатель худу"
L["Hoptallus"] = "Прыгопотам"
L["Horde Berserker"] = "Ордынский берсерк"
L["Horde Cleric"] = "Ордынский священник"
L["Horde Ranger"] = "Ордынский следопыт"
L["Houndmaster Braun"] = "Псарь Браун"
L["Hourglass Cannon"] = "Пушка Песочных часов"
L["Hourglass Tidesage"] = "Заклинатель волн с Песочных часов"
L["Howling Gale"] = "Воющая буря"
L["Hozen Party Animal"] = "Озверевший хозен-тусовщик"
L["Hulking Berserker"] = "Огромный берсерк"
L["Hulking Bloodguard"] = "Громадный кровостраж"
L["Hulking Warshell"] = "Бронированный боец – гигант"
L["Humanoid"] = "Гуманоид"
L["Hungry Lasher"] = "Голодный плеточник"
L["Hylbrande"] = "Хильбранд"
L["Hymdall"] = "Химдалль"
L["Hyrja"] = "Хирья"
L["icc"] = "Цитадель Ледяной Короны"
L["iccFloor1"] = "Нижний ярус"
L["iccFloor2"] = "Черепной вал"
L["iccFloor3"] = "Подъем Смертоносного"
L["iccFloor4"] = "Логово Королевы Льда"
L["iccFloor5"] = "Верхний ярус"
L["iccFloor6"] = "Королевские палаты"
L["iccFloor7"] = "Ледяной Трон"
L["iccFloor8"] = "Ледяная Скорбь"
L["Ichoron"] = "Гнойрон"
L["Ickor Bileflesh"] = "Икор Желчная Плоть"
L["If the Minimap Button is enabled"] = "Если кнопка миникарты включена"
L["Ignis the Furnace Master"] = "Повелитель горнов Игнис"
L["Illysanna Ravencrest"] = "Иллисанна Гребень Ворона"
L["Imacu'tya"] = "Имаку'туя"
L["Imbued Stormcaller"] = "Усиленный призыватель шторма"
L["imbuedIronBarDescription"] = "Может использоваться воинами, гномами и кузнецами Каз Алгар (25)"
L["Immoliant Fury"] = "Яростный пронзатель"
L["Import"] = "Импортировать"
L["Import a preset from a text string"] = "Импортировать маршрут из текстовой строки"
L["Import Data"] = "Импорт данных"
L["Import Preset"] = "Импорт маршрута"
L["Imprison"] = "Пленение"
L["In the bottom right corner"] = "В правом нижнем углу"
L["Incapacitate"] = "Паралич"
L["Incinerator Arkolath"] = "Испепелитель Арколат"
L["incompatibleVersionError"] = "Эта версия World of Warcraft несовместима с Mythic Dungeon Tools."
L["Inconspicuous Plant"] = "Непримечательное растение"
L["Increase Brush Size"] = "Увеличить размер кисти"
L["Infected Bear"] = "Зараженный медведь"
L["Infected Lasher"] = "Зараженный плеточник"
L["Infected Peasant"] = "Зараженный крестьянин"
L["Infectious Ghoul"] = "Заразный вурдалак"
L["Infernal Imp"] = "Инфернальный бес"
L["Infested"] = "Зараженный"
L["Infested Icecaller"] = "Зараженная сотворительница льда"
L["Infinite Adversary"] = "Неприятель из рода Бесконечности"
L["Infinite Agent"] = "Посланница из рода Бесконечности"
L["Infinite Chronoweaver"] = "Ткачиха времени из рода Бесконечности"
L["Infinite Diversionist"] = "Диверсант из рода Бесконечности"
L["Infinite Hunter"] = "Охотник из рода Бесконечности"
L["Infinite Infiltrator"] = "Лазутчик из рода Бесконечности"
L["Infinite Keeper"] = "Хранитель из рода Бесконечности"
L["Infinite Riftmage"] = "Маг портала бесконечности"
L["Infinite Saboteur"] = "Саботажник из рода Бесконечности"
L["Infinite Slayer"] = "Душегуб из рода Бесконечности"
L["Infinite Timebender"] = "Исказительница времени из рода Бесконечности"
L["Infinite Timereaver"] = "Попиратель времен из рода Бесконечности"
L["Infinite Timeslicer"] = "Рассекатель времени из рода Бесконечности"
L["Infinite Twilight Magus"] = "Сумеречный волхв из рода Бесконечности"
L["Infinite Watchkeeper"] = "Караульный из рода Бесконечности"
L["Infinite Whelp"] = "Дракончик из рода Бесконечности"
L["Inflamed Hozen Brawler"] = "Рассвирепевший хозен-буян"
L["Infused Pyromancer"] = "Наполненный силой пиромант"
L["Infused Quill-feather"] = "Зачарованное перо"
L["Infused Whelp"] = "Заряженный дракончик"
L["Infuser Sariya"] = "Насыщательница Сария"
L["Infuser's Rotunda"] = "Ротонда Насыщателя"
L["InfusersRotunda"] = "Ротонда Насыщателя"
L["Infusion Chamber"] = "Зал Насыщения"
L["Infusion Chambers"] = "Залы насыщения"
L["InfusionChamber"] = "Залы насыщения"
L["Ingra Maloch"] = "Ингра Малох"
L["Ingvar the Plunderer"] = "Ингвар Расхититель"
L["Ink of Ozumat"] = "Чернила Озумата"
L["Inquisitor Sigar"] = "Инквизитор Зигар"
L["Inquisitor Tormentorum"] = "Инквизитор Истязарий"
L["Insatiable Brute"] = "Ненасытный громила"
L["Insert Note"] = "Добавить примечание"
L["Inspiring"] = "Воодушевляющий"
L["Instructor Chillheart"] = "Инструктор Ледяное Сердце"
L["Instructor Razuvious"] = "Инструктор Разувий"
L["Interment Construct"] = "Погребальный голем"
L["Interrogation Specialist"] = "Специалист по допросу"
L["Interruptible"] = "Прерываемый"
L["Invading Mite"] = "Клещ-захватчик"
L["Invalid import string"] = "Неправильная строка импорта"
L["Invigorating Fish Stick"] = "Бодрящий рыбный тотем"
L["Invoked Shadowflame Spirit"] = "Пробужденный пламенный дух"
L["Ionar"] = "Ионар"
L["I'pa"] = "И'па"
L["Iridikron"] = "Иридикрон"
L["Iridikron's Creation"] = "Творение Иридикрона"
L["Iron Infantry"] = "Железный пехотинец"
L["Iron Mender"] = "Железный ремонтник"
L["Iron Star"] = "Железная звезда"
L["IronDocks"] = "Железные доки"
L["ironDocksIronStar"] = "Железная звезда"
L["ironDocksShortName"] = "ЖД"
L["Ironhull Apprentice"] = "Ученик Айронхалла"
L["Ironroot Lasher"] = "Железокорневый плеточник"
L["Irontide Bonesaw"] = "Мясник из братства Стальных Волн"
L["Irontide Buccaneer"] = "Буканьер из братства Стальных Волн"
L["Irontide Cleaver"] = "Рассекатель из братства Стальных Волн"
L["Irontide Corsair"] = "Корсар из братства Стальных Волн"
L["Irontide Crackshot"] = "Стрелок из братства Стальных Волн"
L["Irontide Crusher"] = "Сокрушитель из братства Стальных Волн"
L["Irontide Curseblade"] = "Проклятый клинок из братства Стальных Волн"
L["Irontide Enforcer"] = "Головорез из братства Стальных Волн"
L["Irontide Marauder"] = "Мародер из братства Стальных Волн"
L["Irontide Mastiff"] = "Мастиф братства Стальных Волн"
L["Irontide Oarsman"] = "Боец на веслах из братства Стальных Волн"
L["Irontide Officer"] = "Офицер из братства Стальных Волн"
L["Irontide Powdershot"] = "Фузилер из братства Стальных Волн"
L["Irontide Raider"] = "Налетчик из братства Стальных Волн"
L["Irontide Ravager"] = "Опустошитель из братства Стальных Волн"
L["Irontide Stormcaller"] = "Призыватель шторма из братства Стальных Волн"
L["Irontide Thug"] = "Громила из братства Стальных Волн"
L["Irontide Waveshaper"] = "Заклинатель моря из братства Стальных Волн"
L["Irontorch Commander"] = "Командир Железного Факела"
L["Ironwing Flamespitter"] = "Железнокрылый изрыгатель огня"
L["Ivanyr"] = "Иванир"
L["Ixin"] = "Иксин"
L["Ixkreten The Unbreakable"] = "Икскретен Несокрушимая"
L["Izo The Grand Splicer"] = "Изо Великая Сращивательница"
L["Jabbing Flyer"] = "Летун-пронзатель"
L["Jagged Hound"] = "Шипастая гончая"
L["Jandice Barov"] = "Джандис Баров"
L["Jazshariu"] = "Джазшариу"
L["Jedoga Shadowseeker"] = "Джедога Искательница Теней"
L["Jes Howlis"] = "Джес Хаулис"
L["Jiang"] = "Цзян"
L["Join"] = "Присоединиться"
L["Join Crew"] = "Присоединяйтесь к команде"
L["Join Live Session"] = "Присоединиться к 'Прямой трансляции':%s%s: %s - %s"
L["Junkyard D.0.G."] = "Сторожевой бот модели ПЕС"
L["Juvenile Runestag"] = "Молодой рунический олень"
L["K.U.-J.0."] = "КУ-ДЖ0"
L["karaLowerShortName"] = "КАРАЖАН"
L["karaUpperShortName"] = "КАРАЖАН"
L["Kargesh Highguard"] = "Великий страж клана Каргеш"
L["Kargesh Ribcrusher"] = "Крушитель ребер клана Каргеш"
L["Keeza Quickfuse"] = "Киза Скоропышец"
L["Kel'Thuzad"] = "Кел'Тузад"
L["Keristrasza"] = "Керистраза"
L["Khajin the Unyielding"] = "Каджин Непреклонная"
L["Kikatal The Harvester"] = "Ки'катал Жница"
L["King"] = "Король"
L["King A'akul"] = "Король А'акул"
L["King Bjorn"] = "Король Бьорн"
L["King Dazar"] = "Король Дазар"
L["King Deepbeard"] = "Король Волнобород"
L["King Dred"] = "Король Дред"
L["King Gobbamak"] = "Король Гоббамак"
L["King Haldor"] = "Король Галдор"
L["King Rahu'ai"] = "Король Рау'ай"
L["King Ranulf"] = "Король Ранульф"
L["King Timalji"] = "Король Тималджи"
L["King Tor"] = "Король Тор"
L["King Ymiron"] = "Король Имирон"
L["Kings' Rest"] = "Гробница королей"
L["Kings' Rest Sublevel"] = "Гробница королей (нижний уровень)"
L["kingsRestShortName"] = "ГК"
L["Kin-Tara"] = "Кин-Тара"
L["Klotos"] = "Клот"
L["Knight Captain Valyri"] = "Рыцарь-капитан Валири"
L["Knock"] = "Сбивание с ног"
L["Kobold Flametender"] = "Кобольд – хранитель огня"
L["Kobold Taskworker"] = "Кобольд-служитель"
L["Kokia Blazehoof"] = "Кокия Пламенное Копыто"
L["Kologarn"] = "Кологарн"
L["Koramar"] = "Корамар"
L["Krastinovian Carver"] = "Терзатель Крастинова"
L["krBrutePatrolNote"] = "Этот Брут перестанет патрулировать, когда дойдет до Посланницы глубин 7"
L["krGraveyardNote1"] = "Открывается после победы над Советом племен"
L["Krik'thik Demolisher"] = "Крик'тик-подрывник"
L["Krik'thik Infiltrator"] = "Крик'тик-разведчик"
L["Krik'thik Rager"] = "Крик'тик-яростень"
L["Krik'thik Sapper"] = "Крик'тик-сапер"
L["Krik'thik Wind Shaper"] = "Крик'тик - направитель ветров"
L["Krik'thir the Gatewatcher"] = "Крик'тир Хранитель Врат"
L["Krolusk Hatchling"] = "Детеныш кролуска"
L["Krolusk Pup"] = "Подрастающий кролуск"
L["krSkipNote"] = "Непорочный дух - проводник%sДоступен после убийства Голема-чистильщика 1"
L["Krystallus"] = "Кристаллус"
L["Kryxis the Voracious"] = "Криксис Ненасытный"
L["Kuai the Brute"] = "Куай Бесчеловечный"
L["Kul Tiran Footman"] = "Кул-тирасский пехотинец"
L["Kul Tiran Halberd"] = "Кул-тирасский стражник"
L["Kul Tiran Marksman"] = "Кул-тирасский стрелок"
L["Kul Tiran Vanguard"] = "Кул-тирасский боец авангарда"
L["Kul Tiran Wavetender"] = "Кул-тирасский смотритель приливов"
L["Kula the Butcher"] = "Кула Живодерка"
L["Kul'tharok"] = "Кул'тарок"
L["Kur'talos Ravencrest"] = "Кур'талос Гребень Ворона"
L["Kyrakka"] = "Киракка"
L["Kyrian Dark-Praetor"] = "Кирийский темный претор"
L["Kyrian Stitchwerk"] = "Кирийский кадавр"
L["Kyrioss"] = "Кириосс"
L["Lady Baihu"] = "Леди Байху <Кирин-торский командир>"
L["Lady Blaumeux"] = "Леди Бломе"
L["Lady Hatecoil"] = "Леди Кольцо Ненависти"
L["Lady Naz'jar"] = "Леди Наз'жар"
L["Lady Velandras Ravencrest"] = "Леди Веландра Гребень Ворона"
L["Lady Waycrest"] = "Леди Уэйкрест"
L["Lakesis"] = "Лакезис"
L["Language"] = "Язык"
L["LargePresetWarning"] =
  "Вы пытаетесь поделиться очень большим маршрутом (время передачи: %.1fs)%s. Рекомендуется использовать функцию экспорта и делиться большими маршрутами через внешние веб-сайты или удалять рисунки из маршрута.%sВы уверены, что хотите поделиться этим маршрутом?%s"
L["Lashing Voidling"] = "Порождение Бездны"
L["Lava Flare"] = "Вспышка лавы"
L["Lavanthor"] = "Лавантор"
L["Leaping Spark"] = "Скачущая искра"
L["Legion"] = "Легион"
L["Legion Hound"] = "Гончая Легиона"
L["Lerai, Timesworn Maiden"] = "Лерай Служительница Времени"
L["Lesser Sha"] = "Малый ша"
L["Level %d %s"] = "Уровень %d %s"
L["Levels below 10 will hide enemies related to seasonal affixes"] =
  "В подземельях ниже 10-го уровня враги, относящиеся к сезонному аффиксу, будут скрыты"
L["Ley-Guardian Eregos"] = "Хранитель энергии Эрегос"
L["Leymor"] = "Леймор"
L["Library Floor"] = "Библиотека Хранителя"
L["Life Warden Gola"] = "Страж жизни Гола"
L["Lightning Charged Iron Dwarf"] = "Заряженный молнией железный дворф"
L["Lightning Construct"] = "Создание молнии"
L["Lightspawn"] = "Порождение Света"
L["Lilian Voss"] = "Лилиан Восс"
L["Link Spells"] = "Поделиться заклинаниями"
L["Liu Flameheart"] = "Лю Пламенное Сердце"
L["Live"] = "Прямая трансляция"
L["Live Session"] = "Прямая трансляция"
L["Living Current"] = "Живой поток"
L["Living Mojo"] = "Живое колдунство"
L["Living Monstrosity"] = "Живое чудовище"
L["Living Rot"] = "Живая гниль"
L["Living Waste"] = "Живые отходы"
L["Loaderbot"] = "Бот-погрузчик"
L["Loatheb"] = "Лотхиб"
L["Local color blind mode"] = "Частичный режим для дальтоников"
L["localeButtonTooltip1"] = "Требуется дополнение AddonLocale"
L["localeButtonTooltip2"] = "Нажмите, чтобы запустить диалоговое окно настроек в чате"
L["Locked"] = "Заперто"
L["Loken"] = "Локен"
L["Lord Chamberlain"] = "Лорд-камергер"
L["Lord Etheldrin Ravencrest"] = "Лорд Этелдрин Гребень Ворона"
L["Lord Ravencrest's Chamber"] = "Комната лорда Гребня Ворона"
L["Lord Stormsong"] = "Лорд Штормсонг"
L["Lord Waycrest"] = "Лорд Уэйкрест"
L["Lost Soul"] = "Заблудшая душа"
L["Loszkeleth"] = "Лозкелет"
L["Lower Broken Stair"] = "Низ разрушенной лестницы"
L["Lower Pinnacle"] = "Нижняя вершина"
L["LowerChamber"] = "Нижний зал"
L["Lowly Moleherd"] = "Кротопас-плебей"
L["Loyal Creation"] = "Верное создание"
L["Loyal Stoneborn"] = "Верный камнерожденный"
L["Lubricator"] = "Смазчик"
L["Ludwig Von Tortollan"] = "Людвиг фон Тортоллен"
L["L'ura"] = "Л'ура"
L["Lurking Tempest"] = "Затаившаяся буря"
L["Mad Scientist"] = "Безумный ученый"
L["Maddened Survivalist"] = "Обезумевший мастер выживания"
L["Maexxna"] = "Мексна"
L["Mage Hunter Ascendant"] = "Опытный охотник на магов"
L["Mage Hunter Initiate"] = "Посвященный охотник на магов"
L["Mage Slayer"] = "Убийца магов"
L["Mage-Lord Urom"] = "Маг-лорд Уром"
L["MaggotNote"] =
  "Примечание относительно Прожорливых личинок:%s%sПрожорливые личинки с баффом 'Паразитические' будут пытаться «заразить» игроков%sПосле успешного применения 'Заражения' Прожорливые личинки исчезнут и создадут 2-x Прожорливых личинок после того, как закончится дебафф на зараженном игроке. %sВы можете получить только 1 очко за убийство первоначальной Зараженной личинки - 2 недавно появившихся Зараженных личинки не дают очки.%s%sЗараженные крестьяне порождают 3-x Прожорливых личинок, которые дают 1 очко каждая.%sЭти Прожорливые личинки отображаются рядом с Зараженными крестьянами"
L["Magic"] = "Магия"
L["Magma Rager"] = "Магматический яростень"
L["Magmatusk"] = "Магмобивень"
L["Maiden of Grief"] = "Дева Скорби"
L["Maiden of Virtue"] = "Благочестивая дева"
L["Make this preset the live preset"] = "Сделать этот маршрут активным в 'Прямом эфире'"
L["Make window transparent in combat"] = "Сделать окно прозрачным, когда Вы в бою"
L["Makogg Emberblade"] = "Макогг Угольный Клинок"
L["Mak'rana Hardshell"] = "Твердопанцирник Мак'раны"
L["Mak'rana Siltwalker"] = "Ходульник Мак'раны"
L["Malfunctioning Scrapbot"] = "Неисправный хламобот"
L["Mal'Ganis"] = "Мал'Ганис"
L["Malignant Defiler"] = "Злонравный осквернитель"
L["Malignant Spawn"] = "Тлетворное порождение"
L["Mana Confluence"] = "Скопление маны"
L["Mana Devourer"] = "Пожиратель маны"
L["Mana Saber"] = "Манапард"
L["Mana Wyrm"] = "Маназмей"
L["manaDevourerNote"] =
  "Пожиратель маны 1 является боссом, потому что он также даёт награду за вражеские силы"
L["Mana-Gorged Wyrm"] = "Насытившийся маназмей"
L["Mangrove Ent"] = "Мангровый энт"
L["Maniacal Soulbinder"] = "Маниакальный стражник душ"
L["Manifestation of Envy"] = "Воплощение зависти"
L["Manifested Shadow"] = "Призванная тень"
L["Manifested Timeways"] = "Ожившие потоки времени"
L["Marauding Geist"] = "Разбойничающий упырь"
L["Margrave Stradama"] = "Маркграфиня Страдама"
L["Marked Sister"] = "Меченая сестра"
L["Market Peacekeeper"] = "Тазавешский хранитель порядка"
L["Maruuk"] = "Маруук"
L["Master Archer"] = "Искусный лучник"
L["Master Necromancer"] = "Мастер некромантии"
L["Master Snowdrift"] = "Мастер Снежный Вихрь"
L["Master's Terrace"] = "Терраса Медива"
L["Matron Alma"] = "Матрона Альма"
L["Matron Bryndle"] = "Матрона Бриндл"
L["Mature Krolusk"] = "Взрослый кролуск"
L["MausoleumOfLegends"] = "Мавзолей Легенд"
L["Maw of Souls"] = "Утесы Адской Пасти"
L["mawOfSoulsShortName"] = "УАП"
L["Mchimba the Embalmer"] = "Мчимба Бальзамировщик"
L["MDI Mode"] = "Режим MDI"
L["MDT Error"] = "Ошибка MDT"
L["MDT: Cannot add enemy - you are trying to add too many enemies of the same kind"] =
  "MDT: Невозможно добавить противника. Вы пытаетесь добавить слишком много противников одного типа"
L["MDT: Error importing preset"] = "MDT: Ошибка импорта маршрута"
L["MDT: Spells for %s:"] = "MDT: Заклинания для %s:"
L["MDT: Use /mdt minimap to show the minimap icon again"] = "MDT: используйте /mdt minimap, чтобы снова отобразить значок миникарты"
L["MDTGuideNote"] = "MDTGuide не связан с Mythic Dungeon Tools и является отдельным дополнением."
L["Meathook"] = "Мясной Крюк"
L["Mech Jockey"] = "Механожокей"
L["Mechadrone Sniper"] = "Мехадрон-снайпер"
L["Mechagon"] = "Мехагон"
L["Mechagon - Junkyard"] = "Операция Мехагон: Свалка"
L["Mechagon - Workshop"] = "Операция Мехагон: Местерская"
L["Mechagon Cavalry"] = "Мехагонский кавалерист"
L["Mechagon Citizen"] = "Житель Мехагона"
L["Mechagon City"] = "Мехагон"
L["Mechagon Island"] = "Остров Мехагон"
L["Mechagon Island (Tunnels)"] = "Остров Мехагон (тоннели)"
L["Mechagon Mechanic"] = "Мехагонский механик"
L["Mechagon Prowler"] = "Мехагонский хищник"
L["Mechagon Renormalizer"] = "Мехагонский нормализатор"
L["Mechagon Tinkerer"] = "Мехагонский боевой механик"
L["Mechagon Trooper"] = "Мехагонский солдат"
L["mechagonCityShortName"] = "МЕХАГОН"
L["mechagonIslandShortName"] = "СВАЛКА"
L["mechagonWorkshopShortName"] = "МАСТЕР"
L["Mechanical"] = "Механизм"
L["Mechanized Peacekeeper"] = "Механомиротворец"
L["Melded Berserker"] = "Зараженный берсерк"
L["Melidrussa Chillworn"] = "Мелидрусса Истощенная Холодом"
L["Menial Laborer"] = "Разнорабочий"
L["Mephistroth"] = "Мефистрот"
L["Merektha"] = "Меректа"
L["Metal Gunk"] = "Металлическая жижа"
L["Middle-click to disable Minimap Button"] = "[СКМ] - отключить кнопку миникарты"
L["Midnight"] = "Полночь"
L["Mightstone Breaker"] = "Крушитель из племени Камня Силы"
L["Millhouse Manastorm"] = "Миллхаус Манашторм"
L["Millificent Manastorm"] = "Миллифисент Манашторм"
L["Mimiron"] = "Мимирон"
L["Mind Control"] = "Контроль над разумом"
L["Mind Soothe"] = "Успокоение разума"
L["Mindbender Ghur'sha"] = "Подчиняющий разум Гур'ша"
L["Mindless Servant"] = "Безмозглый слуга"
L["Mindshattered Screecher"] = "Бешеная визгунья"
L["Mine Rat"] = "Кобольд-рудокоп"
L["minecartNote"] = "Вагонетка%sМожет быть использована игроками"
L["Ming the Cunning"] = "Мин Коварный"
L["Minion of Doubt"] = "Служитель Сомнения"
L["Minion of Ghur'sha"] = "Прислужник Гур'ши"
L["Minion of Zul"] = "Прислужник Зула"
L["Minister of Air"] = "Служитель воздуха"
L["Mire Soldier"] = "Болотный солдат"
L["Misguided Nymph"] = "Заблудшая нимфа"
L["Mistcaller"] = "Призывательница Туманов"
L["Mists of Tirna Scithe"] = "Туманы Тирна Скитта"
L["mistsShortName"] = "ТТС"
L["Mistveil Defender"] = "Туманная защитница"
L["Mistveil Gorgegullet"] = "Жоробрюх Туманной Завесы"
L["Mistveil Guardian"] = "Туманный страж"
L["Mistveil Matriarch"] = "Матриарх темнокрылов"
L["Mistveil Nightblossom"] = "Ночноцвет Туманной Завесы"
L["Mistveil Shaper"] = "Туманный культиватор"
L["Mistveil Stalker"] = "Туманный хищник"
L["Mistveil Stinger"] = "Туманный острожал"
L["Mistveil Tender"] = "Туманная хранительница"
L["mlGraveyardNote1"] = "Откроется после победы над Платным разгонятелем толпы"
L["mlGraveyardNote2"] = "Открывается после завершения поездки на вагонетке"
L["mlGraveyardNote3"] = "Откроется после победы над Риксой Огневерт"
L["mlJockeyNote"] = "Механожокей 21 и 22 должны быть помечены в момент уничтожения их миротворцев"
L["Mogul Razdunk"] = "Шеф Разданк"
L["Mogu'shan Palace"] = "Дворец Могу'шан"
L["mogushanPalaceShortName"] = "ДМШ"
L["Molten Colossus"] = "Оплавленный колосс"
L["Molten Giant"] = "Оплавленный великан"
L["Monstrous Corpse Spider"] = "Чудовищный трупный паук"
L["Monstrous Decay"] = "Гнилое чудовище"
L["Monzumi"] = "Монзуми"
L["Moorabi"] = "Мураби"
L["MoP Challenge Mode"] = "Режим испытания Пандарии"
L["Moragg"] = "Морагг"
L["Morchie"] = "Морхи"
L["Mordretha, the Endless Empress"] = "Мордрета, Вечная императрица"
L["Moroes"] = "Мороуз"
L["motherlodeShortName"] = "ЖИЛА"
L["Mouseover a patrolling enemy with a blue border to view the patrol path."] =
  "Наведите указатель мыши на патрулирующего врага с синей рамкой, чтобы увидеть его путь."
L["Mouseover the Live button while in a group to learn more about Live mode."] =
  "Наведите указатель мыши на кнопку 'Прямая трансляция', находясь в группе, чтобы узнать больше о 'Прямом эфире'."
L["Move Object"] = "Переместить объект"
L["Mueh'zala"] = "Мве'зала"
L["Murkbrine Fishmancer"] = "Рыбомант из племени Соленой Хмари"
L["Murkbrine Scalebinder"] = "Лататель чешуи из племени Соленой Хмари"
L["Murkbrine Shellcrusher"] = "Крушитель панцирей из племени Соленой Хмари"
L["Murkbrine Shorerunner"] = "Береговой бегун из племени Соленой Хмари"
L["Murkbrine Wavejumper"] = "Волнопляс из племени Соленой Хмари"
L["Mu'Shiba"] = "Му'Шиба"
L["Mutated Hatchling"] = "Сумеречный детеныш"
L["Mystic Ssa'veh"] = "Мистик Сса'ве"
L["Mythresh, Sky's Talons"] = "Мифреш Небесный Коготь"
L["Naeno Megacrash"] = "Нано Мегабум"
L["Nakt"] = "Накт"
L["Nal'asha"] = "Нал'аша"
L["Nalthor the Rimebinder"] = "Налтор Криомант"
L["Nal'tira"] = "Нал'тира"
L["Naraxas"] = "Нараксас"
L["Nar'zudah"] = "Нар'зуда"
L["Nathrezim Infiltrator"] = "Натрезим-лазутчик"
L["Nature's Blade"] = "Клинок природы"
L["Naxxramas"] = "Наксрамас"
L["Naxxramas Acolyte"] = "Послушник из Наксрамаса"
L["Naxxramas Cultist"] = "Служитель культа из Наксрамаса"
L["Naz'jar Frost Witch"] = "Ледяная ведьма леди Наз'жар"
L["Naz'jar Honor Guard"] = "Отважный страж Леди Наз'жар"
L["Naz'jar Invader"] = "Захватчик Леди Наз'жар"
L["Naz'jar Oracle"] = "Оракул леди Наз'жар"
L["Naz'jar Ravager"] = "Опустошительница леди Наз'жар"
L["Naz'jar Sentinel"] = "Часовой Леди Наз'жар"
L["Naz'jar Tempest Witch"] = "Ведьма бурь Леди Наз'жар"
L["Necro Knight"] = "Некрорыцарь"
L["Necrotic"] = "Некротический"
L["Necrotic Spiderling"] = "Некротический паучок"
L["necroticWakeShortName"] = "СТ"
L["Neesa Nox"] = "Нииза Нокс"
L["Nefarious Darkspeaker"] = "Пагубный темный проповедник"
L["Nekthara the Mangler"] = "Нектара Изувер"
L["Neltharion's Lair"] = "Логово Нелтариона"
L["Neltharion's Lair Sublevel"] = "Логово Нелтариона"
L["neltharionsLairShortName"] = "ЛН"
L["Neltharus"] = "Нелтарий"
L["neltharusChain"] = "Горящая цепь"
L["neltharusShortName"] = "НЕЛТ"
L["Nerubian Hauler"] = "Нерубская погрузчица"
L["Ner'zhul"] = "Нер'зул"
L["Netherspace"] = "Пустомарь"
L["New"] = "Новый"
L["New NPC at Cursor Position"] = "Новый НИП на позиции курсора"
L["New Patrol Waypoint at Cursor Position"] = "Новая точка направления патруля на позиции курсора"
L["New Preset"] = "Новый маршрут"
L["Next to the NPC"] = "Рядом с НИП"
L["Nhallish"] = "Нхаллиш"
L["Night Watch Mariner"] = "Моряк из Ночного дозора"
L["Nightborne Reclaimer"] = "Ночнорожденный-возродитель"
L["Nightborne Spellsword"] = "Ночнорожденный - заклинатель клинка"
L["Nightfall Commander"] = "Сумеречный командир"
L["Nightfall Curseblade"] = "Сумеречный проклятый клинок"
L["Nightfall Dark Architect"] = "Сумеречный архитектор тьмы"
L["Nightfall Darkcaster"] = "Сумеречная заклинательница тьмы"
L["Nightfall Ritualist"] = "Сумеречный ритуалист"
L["Nightfall Shadowalker"] = "Сумеречный странник теней"
L["Nightfall Shadowmage"] = "Сумеречный маг тени"
L["Nightfall Tactician"] = "Сумеречный тактик"
L["Nightmare Dweller"] = "Обитатель Кошмара"
L["Nitrogg Thundertower"] = "Нитрогг Громовая Гора"
L["Njorndir Preparation"] = "Подготовка Ньорндира"
L["Noble Skirmisher"] = "Аристократ-дуэлянт"
L["noDungeonData1"] = "Кажется, у Вас еще нет данных о подземельях."
L["noDungeonData2"] = "Создайте свои собственные данные через /mdt devmode"
L["noDungeonData3"] =
  "В качестве альтернативы узнайте, как получить дополнительные данные о подземелье здесь:"
L["Nokhud Beastmaster"] = "Повелительница зверей из клана Нокхуд"
L["Nokhud Defender"] = "Заступник из клана Нокхуд"
L["Nokhud Hornsounder"] = "Трубач из клана Нокхуд"
L["Nokhud Houndsman"] = "Псарь из клана Нокхуд"
L["Nokhud Lancemaster"] = "Мастер копья из клана Нокхуд"
L["Nokhud Longbow"] = "Лучница из клана Нокхуд"
L["Nokhud Neophyte"] = "Неофит из клана Нокхуд"
L["Nokhud Plainstomper"] = "Равнинный топотун из клана Нокхуд"
L["Nokhud Saboteur"] = "Нокхудский диверсант"
L["Nokhud Stormcaster"] = "Буремант из клана Нокхуд"
L["Nokhud Thunderfist"] = "Громовой кулак из клана Нокхуд"
L["Nokhud Villager"] = "Кентавр из клана Нокхуд"
L["Nokhud Warhound"] = "Бойцовский пес клана Нокхуд"
L["Nokhud Warsmith"] = "Боевой кузнец из клана Нокхуд"
L["Nokhud Warspear"] = "Боевое копье клана Нокхуд"
L["nokhudOffensiveShortName"] = "НКН"
L["None"] = "Нет"
L["Not specified"] = "Не указано"
L["Note Text:"] = "Текст примечания:"
L["Noth the Plaguebringer"] = "Нот Чумной"
L["Novos the Summoner"] = "Новос Призыватель"
L["NPCs"] = "NPC"
L["Nullmagic Hornswog"] = "Рогоплав-магоед"
L["Nx"] = "Нкс <Клык королевы>"
L["Oakheart"] = "Дубосерд"
L["Oasis Security"] = "Охранник Оазиса"
L["Odyn"] = "Один"
L["Off-Duty Laborer"] = "Отдыхающий работник"
L["Officer Quarters"] = "Офицерская"
L["Ogron Laborer"] = "Огрон-работник"
L["Okay"] = "Ok"
L["Ol' Waxbeard"] = "Старый Воскобород"
L["Olaf"] = "Олаф"
L["Olmyr the Enlightened"] = "Олмир Просвещенный"
L["Omega Buster"] = "Омега-крушитель"
L["Ook-Ook"] = "Ук-Ук"
L["Oozing Leftovers"] = "Сочащиеся ошметки"
L["Open an issue on GitHub"] = "Открыть тикет о проблеме на GitHub"
L["Open Enemy Info"] = "Открыть информацию о враге"
L["Open MDI override options"] = "Открыть опции MDI поверх"
L["openSettingsTooltip"] = "Нажмите, чтобы открыть настройки"
L["Opera Hall Balcony"] = "Балкон в опере"
L["Opera Hall: Wikket"] = "Оперный зал: Сцена"
L["Operation: Floodgate"] = "Операция: Шлюз"
L["operationFloodgateShortName"] = "ШЛЮЗ"
L["Oppressive Banner"] = "Подавляющее знамя"
L["Orator Krixvizk"] = "Оратор Крикс'визк"
L["Ordnance Specialist"] = "Артиллерист"
L["Ore Elemental"] = "Элементаль руды"
L["Ormorok the Tree-Shaper"] = "Орморок Воспитатель Дерев"
L["Oros Coldheart"] = "Орос Бессердечный"
L["Oryphrion"] = "Орифрион"
L["Oshir"] = "Ошир"
L["Ossified Conscript"] = "Покрытый оссеином рядовой"
L["Overcharged Sentinel"] = "Перегруженный часовой"
L["Overflowing"] = "Переполняющий"
L["Overgorged Mite"] = "Объевшийся клещ"
L["Overgrown Ancient"] = "Заросшее древо"
L["Overgrown Roots"] = "Разросшиеся корни"
L["overgrownRootsDescription"] =
  "Может использоваться ночными эльфами, тауренами, тауренами Крутогорья, друидами и травниками Каз Алгар (25)"
L["Overloaded Mailemental"] = "Перегруженный почтоменталь"
L["Overseer Korgus"] = "Надзиратель Корги"
L["Overseer Lahar"] = "Надзиратель Лахар"
L["Overseer's Redoubt"] = "Цитадель надзирателя"
L["Overseer's Summit"] = "Терраса надзирателя"
L["Ozumat"] = "Озумат"
L["P.O.S.T. Master"] = "ПОЧТ-мейстер"
L["P.O.S.T. Worker"] = "Служащий ПОЧТ-ы"
L["Paceran the Virulent"] = "Пакиран Заразный"
L["Pack Mole"] = "Вьючный крот"
L["Paladin of the Silver Hand"] = "Паладин ордена Серебряной Длани"
L["Pallid Gorger"] = "Бледный пожиратель"
L["Parts Recovery Technician"] = "Техник по обломкам"
L["Patchwerk"] = "Лоскутик"
L["Patchwerk Soldier"] = "Лоскутный солдат"
L["Patchwork Construct"] = "Лоскутное создание"
L["Patchwork Golem"] = "Лоскутный голем"
L["Path of Illumination"] = "Путь Озарения"
L["Patrol Captain Gerdo"] = "Капитан патруля Гердо"
L["Pendule"] = "Маятник"
L["Peril"] = "Опасность"
L["Pestilence Slime"] = "Моровая слизь"
L["Pestilent Harvester"] = "Заразный добытчик"
L["Phantasmal Air"] = "Ирреальный воздух"
L["Phantasmal Cloudscraper"] = "Ирреальный туческреб"
L["Phantasmal Fire"] = "Ирреальный огонь"
L["Phantasmal Mammoth"] = "Ирреальный мамонт"
L["Phantasmal Murloc"] = "Ирреальный мурлок"
L["Phantasmal Naga"] = "Ирреальная нага"
L["Phantasmal Ogre"] = "Ирреальный огр"
L["Phantasmal Water"] = "Ирреальная вода"
L["Phantasmal Wolf"] = "Ирреальный волк"
L["Phantom Crew"] = "Фантомный рабочий"
L["Phantom Guardsman"] = "Фантомный стражник"
L["Phantom Guest"] = "Фантомный гость"
L["Pile of Corpses"] = "Куча трупов"
L["Pistonhead Blaster"] = "Стрелок из банды Поршнеголовых"
L["Pistonhead Mechanic"] = "Механик из банды Поршнеголовых"
L["Pistonhead Scrapper"] = "Демонтажница из банды Поршнеголовых"
L["Pitwarden Gwarnok"] = "Страж ям Гварнок"
L["Plague Beast"] = "Чумная тварь"
L["Plague Bomb"] = "Чумная бомба"
L["Plague Doctor"] = "Чумная знахарка"
L["Plague Slime"] = "Чумная слизь"
L["Plague Walker"] = "Чумоброд"
L["Plaguebelcher"] = "Изрыгатель чумы"
L["Plaguebinder"] = "Заклинатель чумы"
L["Plagueborer"] = "Чумной бурильщик"
L["Plaguebound"] = "Чумной труп"
L["Plaguebound Devoted"] = "Чумная прислужница"
L["Plaguebound Fallen"] = "Чумная падшая воительница"
L["Plagued Bat"] = "Зараженная летучая мышь"
L["Plagued Rat"] = "Чумная крыса"
L["Plaguefall"] = "Чумные Каскады"
L["plaguefallDevotedNote"] =
  "Чумные прислужники, которые были трансформированы при помощи умения 'Сочащийся остов' до входа в бой с НИП Икор Желчная Плоть, не засчитываются как силы врага."
L["plaguefallShortName"] = "ЧК"
L["Plagueroc"] = "Чумной рух"
L["Plain Texture"] = "Простая текстура"
L["Players can join the live session by either clicking this button or the Live Session chat link"] =
  "Игроки могут присоединиться к 'Прямой трансляции', нажав на эту кнопку или на ссылку 'Прямой трансляции' в чате"
L["Please report any bugs on https://github.com/adi-no95/MDT_Legion/issues"] =
  "Пожалуйста, сообщайте об ошибках здесь: https://github.com/adi-no95/MDT_Legion/issues"
L["Plundering Geist"] = "Бесчинствующий упырь"
L["Poison"] = "Яд"
L["Poisonous Skitterer"] = "Заразный паук-быстролап"
L["Polymorph"] = "Превращение"
L["Portal Guardian"] = "Страж портала"
L["Portal Keeper"] = "Хранитель портала"
L["Portalmancer Zo'honn"] = "Заклинатель порталов Зо'хонн"
L["Portrait"] = "Портрет"
L["Posh Vacationer"] = "Пафосная курортница"
L["Preset '%s' already exists"] = "Маршрут '%s' уже существует"
L["Preset Export"] = "Экспорт маршрута"
L["Preset Name"] = "Название маршрута"
L["Prideful"] = "Полный гордыни"
L["Priestess Alun'za"] = "Жрица Алун'за"
L["Priestess of Misery"] = "Жрица горя"
L["Primal Gust"] = "Изначальный порыв"
L["Primal Juggernaut"] = "Изначальный исполин"
L["Primal Stormshield"] = "Изначальный щит бури"
L["Primal Terrasentry"] = "Изначальный земляной часовой"
L["Primal Thundercloud"] = "Изначальная грозовая туча"
L["Primal Tsunami"] = "Изначальное цунами"
L["Primalist Arcblade"] = "Воин стихий – искрящий клинок"
L["Primalist Cinderweaver"] = "Пеплоплет воинов стихий"
L["Primalist Earthshaker"] = "Воин стихий – землетряс"
L["Primalist Flamedancer"] = "Воин стихий – танцор пламени"
L["Primalist Galesinger"] = "Воин стихий – певица бури"
L["Primalist Geomancer"] = "Воин стихий – геомант"
L["Primalist Icecaller"] = "Воин стихий – сотворительница льда"
L["Primalist Ravager"] = "Опустошитель воинов стихий"
L["Primalist Shockcaster"] = "Воин стихий – искромаг"
L["Primalist Shocktrooper"] = "Воин стихий – штурмовик"
L["Primalist Stormspeaker"] = "Воин стихий – буревестник"
L["Primalist Thunderbeast"] = "Изначальный громозверь"
L["Prince Keleseth"] = "Принц Келесет"
L["Prince Taldaram"] = "Принц Талдарам"
L["Prioress Murrpray"] = "Настоятельница Муррпрэй"
L["Priory of the Sacred Flame"] = "Приорат Священного Пламени"
L["priory1BossNote"] =
  "Мини-босс, с которым Вы решили сразиться вместе с боссом, не награждается вражескими силами"
L["prioryItemDescription"] = "Может использоваться Жрецами или |cFFF48CBAПаладинами|r"
L["prioryShortName"] = "ПСП"
L["Prison Bars"] = "Тюремные решетки"
L["Prison Gate"] = "Тюремные ворота"
L["Professor Slate"] = "Профессор Слейт"
L["Progenitor Relic"] = "Реликвия Прародителей"
L["Proto-Drake Handler"] = "Укротитель протодраконов"
L["Proto-Drake Rider"] = "Всадник на протодраконе"
L["Provide feedback in Discord"] = "Оставить отзыв в Discord"
L["Pull Drop Clear Pull"] = "Очистить группу"
L["Pull Drop Clear Pulls"] = "Очистить пуллы"
L["Pull Drop Close"] = "Закрыть"
L["Pull Drop Color"] = "Цвет"
L["Pull Drop Color Settings"] = "Настройки цвета"
L["Pull Drop Colorize Preset"] = "Раскрасить маршрут"
L["Pull Drop Delete"] = "Удалить"
L["Pull Drop Insert after"] = "Добавить пулл после"
L["Pull Drop Insert before"] = "Добавить пулл перед"
L["Pull Drop Merge"] = "Объединить"
L["Pull Drop Merge down"] = "Сдвинуть вниз"
L["Pull Drop Merge up"] = "Сдвинуть вверх"
L["Pull Drop Move down"] = "Переместить ниже"
L["Pull Drop Move up"] = "Переместить выше"
L["Pull Drop Reset Color"] = "Сбросить цвет"
L["Pull Drop Reset Preset"] = "Сбросить маршрут"
L["Purification Construct"] = "Голем-чистильщик"
L["Putrid Butcher"] = "Гнилостный мясник"
L["Putrid Pyromancer"] = "Гнилостный пиромант"
L["Qalashi Blacksmith"] = "Куалаши-кузнец"
L["Qalashi Bonesplitter"] = "Куалаши-костедробительница"
L["Qalashi Bonetender"] = "Куалаши-костестраж"
L["Qalashi Hunter"] = "Куалаши-охотник"
L["Qalashi Irontorch"] = "Куалаши-факельщица"
L["Qalashi Lavabearer"] = "Куалаши - воин лавы"
L["Qalashi Lavamancer"] = "Куалаши-лавамант"
L["Qalashi Plunderer"] = "Куалаши-расхититель"
L["Qalashi Spinecrusher"] = "Куалаши-хребтолом"
L["Qalashi Thaumaturge"] = "Куалаши-чудотворица"
L["Qalashi Trainee"] = "Куалаши-новобранец"
L["Qalashi Warden"] = "Куалаши-страж"
L["Quaking"] = "Сотрясающий"
L["Quartermaster Koratite"] = "Интендант Коратит"
L["Queen Patlaa"] = "Королева Патлаа"
L["Queen Wasi"] = "Королева Уаси"
L["Quilen Guardian"] = "Цийлинь-стражник"
L["Raal the Gluttonous"] = "Раал Прожорливый"
L["Radiating Voidstone"] = "Сияющий камень Бездны"
L["Rafters"] = "Стропила"
L["Rageclaw"] = "Яростный Коготь"
L["Raging"] = "Разъяренный"
L["Raging Bloodhorn"] = "Яростный кроворог"
L["Raging Construct"] = "Гневное создание"
L["Raigonn"] = "Райгонн"
L["Railmaster Rocketspark"] = "Рельсмейстер Искролет"
L["Rainbow"] = "Радуга"
L["Rampaging Clefthoof"] = "Буйный копытень"
L["Rancid Gasbag"] = "Тошнотворный газовый мешок"
L["Rank Overseer"] = "Военный надзиратель"
L["Rashanan"] = "Раша'нан"
L["Rat"] = "Крыса"
L["Rattlegore"] = "Громоклин"
L["Ravaging Scavenger"] = "Неистовый падальщик"
L["Ravenous Cinderbee"] = "Хищная золопчела"
L["Ravenous Crawler"] = "Прожорливый ползун"
L["Ravenous Destroyer"] = "Прожорливый разрушитель"
L["Ravenous Dreadbat"] = "Прожорливый жуткий нетопырь"
L["Ravenous Scarab"] = "Прожорливый скарабей"
L["Ravenous Wolf"] = "Прожорливый волк"
L["Ravenshold"] = "Обитель Воронов"
L["Razorscale"] = "Острокрылая"
L["Reanimated Crossbowman"] = "Возрожденный арбалетчик"
L["Reanimated Guardian"] = "Оживленный страж"
L["Reanimated Honor Guard"] = "Оживленный почетный страж"
L["Reanimated Mage"] = "Оживленный маг"
L["Reanimated Ritual Bones"] = "Оживленные ритуальные кости"
L["Reanimated Warrior"] = "Возрожденный воин"
L["Reanimation Totem"] = "Тотем воскрешения"
L["Reaping"] = "Пожинающий"
L["Rear Train Cars"] = "Задние вагоны"
L["receiveErrorUpdate"] =
  "MDT не может правильно получить маршрут. Убедитесь, что у отправителя %s установлена ​​последняя версия MDT!"
L["Receiving: ..."] = "Получение: ..."
L["Red, Green and Blue"] = "Красный, Зелёный и Синий"
L["Redo"] = "Повторить"
L["redoDrawing"] = "Нарисовать заново"
L["Reformed Maiden"] = "Исправившаяся дева"
L["Refreshment Vendor"] = "Продавщица закусок"
L["Refti Custodian"] = "Рефти-распорядитель"
L["Refti Defender"] = "Рефти-защитник"
L["Regal Mistdancer"] = "Благородный туманный танцор"
L["Regenerating Sha"] = "Набирающий силы ша"
L["Reinforced Drone"] = "Укрепленный трутень"
L["Rek the Hardened"] = "Рек Закаленная"
L["Relentless"] = "Непреклонный"
L["Remnant of Fury"] = "Частица неистовства"
L["Rename"] = "Переименовать"
L["Rename Preset"] = "Изменение названия маршрута"
L["Rename the preset"] = "Переименовать маршрут"
L["Repentance"] = "Покаяние"
L["Reposing Knight"] = "Отдыхающий рыцарь"
L["Repurposed Loaderbot"] = "Переделанный бот-погрузчик"
L["Research Scribe"] = "Писец-исследователь"
L["Reset"] = "Сбросить"
L["Reset %s?"] = "Сбросить маршрут %s?"
L["Reset Preset"] = "Сбросить маршрут"
L["Reset the preset to the default state"] = "Сбросить маршрут по умолчанию"
L["Residual Hatred"] = "Остатки Ненависти"
L["Resin Flake"] = "Скопление смолы"
L["Restless Tides"] = "Неутомимая волна"
L["Retired Lord Vul'azak"] = "Бывший владыка Вул'азак"
L["Return to Karazhan Lower"] = "Возвращение в Каражан - нижний ярус"
L["Return to Karazhan Upper"] = "Возвращение в Каражан - верхний ярус"
L["Return to the live preset"] = "Вернуться к маршруту 'в прямом эфире'"
L["Rezan"] = "Резан"
L["Rift Warden"] = "Страж прорыва"
L["Rigged Plagueborer"] = "Взрывоопасный чумной бурильщик"
L["Right click a pull for more options."] = "[ПКМ] - открыть дополнительные параметры."
L["Right click an enemy to open the enemy info window."] = "[ПКМ на противника] - информация о противнике."
L["Right click for more info"] = "<ПКМ> - получить дополнительную информацию"
L["Right-click to lock Minimap Button"] = "[ПКМ] - заблокировать кнопку миникарты"
L["Right-Click to reset NPC position"] = "[ПКМ] - сбросить положение НИП"
L["Ring-Lord Conjurer"] = "Кудесник-кольценосец"
L["Ring-Lord Sorceress"] = "Колдунья-кольценосец"
L["Riptide Shredder"] = "Мастер клинков прилива"
L["Rira Hackclaw"] = "Рира Когтерезка"
L["Risen Arcanist"] = "Восставший чародей"
L["Risen Archer"] = "Восставшая лучница"
L["Risen Bonesoldier"] = "Оживленный костяной воин"
L["Risen Companion"] = "Восставший питомец"
L["Risen Cultist"] = "Восставший сектант"
L["Risen Dragon"] = "Оживший дракон"
L["Risen Drakkari Bat Rider"] = "Восставший всадник на нетопыре из Драккари"
L["Risen Drakkari Death Knight"] = "Восставший рыцарь смерти Драккари"
L["Risen Drakkari Handler"] = "Восставший укротитель Драккари"
L["Risen Drakkari Soulmage"] = "Восставший заклинатель душ из племени Драккари"
L["Risen Drakkari Warrior"] = "Восставший воин Драккари"
L["Risen Footman"] = "Оживший пехотинец"
L["Risen Guard"] = "Восставший стражник"
L["Risen Lancer"] = "Восставший копейщик"
L["Risen Mage"] = "Восставший маг"
L["Risen Mystic"] = "Восставший мистик"
L["Risen Scout"] = "Восставший разведчик"
L["Risen Soul"] = "Восставшая душа"
L["Risen Squire"] = "Восставший рыцарь смерти"
L["Risen Swordsman"] = "Восставший мечник"
L["Risen Warlord"] = "Воскрешенный полководец"
L["Risen Warrior"] = "Восставший воин"
L["Ritual of Bones"] = "Костяной ритуал"
L["Ritualist Lesha"] = "Ритуалистка Лиша"
L["Rixxa Fluxflame"] = "Рикса Огневерт"
L["Rock Smasher"] = "Камнекрушитель"
L["Rockback Gnasher"] = "Камнеспинный щелкозуб"
L["Rockback Snapper"] = "Камнеспинный крепкохват"
L["Rockbound Pelter"] = "Скальный камнешвыр"
L["Rockbound Sprite"] = "Скальный дух"
L["Rockbound Trapper"] = "Скальный зверолов"
L["Rocket Tonk"] = "Ракетный танк"
L["Rockspine Stinger"] = "Камнеспинный жальщик"
L["Rokmora"] = "Рокмора"
L["Rook Spiderling"] = "Паучонок из крепости Черной Ладьи"
L["Rook Spinner"] = "Ткач из крепости Черной Ладьи"
L["rookeryShortname"] = "ГНЕЗДО"
L["rookItemDescription"] =
  "После победы над |cFFFF0000Кириоссом|r добудьте |cFF00FF00Перо буреклюва|r. Используйте его на ближайшем |cFF00FF00Скованном буреклюве|r, чтобы получить этот эффект на 5 минут."
L["Root"] = "Обездвиженность"
L["Rotation"] = "Ротация"
L["Rotbow Ranger"] = "Следопыт гнили"
L["Rotbow Stalker"] = "Стрелок гнили"
L["Rotfang Hyena"] = "Гнилозубая гиена"
L["Rotheart Dryad"] = "Гнилосердная дриада"
L["Rotheart Keeper"] = "Гнилосердный хранитель"
L["Rotmarrow Slime"] = "Костномозговая слизь"
L["Rotspew"] = "Тошногнил"
L["Rotspew Leftovers"] = "Останки Тошногнила"
L["Rotting Maggot"] = "Гнилой червь"
L["Rotting Slimeclaw"] = "Гниющий слизнекоготь"
L["Rowdy Reveler"] = "Буйная гуляка"
L["Royal Acolyte"] = "Королевский прислужник"
L["Royal Jelly Purveyor"] = "Поставщик маточного молочка"
L["Royal Swarmguard"] = "Королевский страж роя"
L["Royal Venomshell"] = "Королевский бронеяд"
L["Royal Wicklighter"] = "Королевский зажигатель"
L["Ruby Overlook"] = "Рубиновый дозор"
L["RubyLifePools"] = "Рубиновые Омуты Жизни"
L["rubyLifePoolsShortName"] = "РОЖ"
L["Ruin's Descent"] = "Спуск скорби"
L["Ruins Dweller"] = "Обитатель руин"
L["Rune Etched Sentry"] = "Покрытый рунами страж"
L["Rune Seal Keeper"] = "Хранительница печати"
L["Runecarver Slave"] = "Порабощенный резчик рун"
L["Runecarver Sorn"] = "Резчик рун Сорн"
L["Runemaster Molgeim"] = "Мастер рун Молгейм"
L["Runestag Elderhorn"] = "Рунический старорог"
L["Runic Disciple"] = "Руническая послушница"
L["Runic Protector"] = "Рунический защитник"
L["Saboteur Kip'tilak"] = "Диверсант Кип'тилак"
L["Sacrificial Pits"] = "Жертвенная яма"
L["Sacristy of Elune"] = "Ризница Элуны"
L["Sadana Bloodfury"] = "Садана Кровавое Безумие"
L["Safety Shark"] = "Сторожевая акула"
L["Salramm the Fleshcrafter"] = "Салрамм Плоторез"
L["Saltscale Lurker"] = "Скрытень из племени Соленой Чешуи"
L["Saltscale Skulker"] = "Тихоступ из племени Соленой Чешуи"
L["Saltsea Droplet"] = "Частица морской воды"
L["Saltwater Snapper"] = "Морская черепаха"
L["Samh'rek, Beckoner of Chaos"] = "Сам'рек Призыватель Хаоса"
L["Sand-Crusted Striker"] = "Песочная змея"
L["Sandfury Stonefist"] = "Крушитель из племени Песчаной Бури"
L["Sandswept Marksman"] = "Песчаный стрелок"
L["Sanguine"] = "Кровавый"
L["Sanguine Cadet"] = "Кровавый кадет"
L["Sanguine Depths"] = "Кровавые Катакомбы"
L["Sanguine DepthsFloor1"] = "Глубины Отчаяния"
L["Sanguine DepthsFloor2"] = "Амфитеатр Скорби"
L["sanguineDepthsShortName"] = "КК"
L["Sap"] = "Ошеломление"
L["Sapped Voidlord"] = "Оглушенный повелитель Бездны"
L["Sapphiron"] = "Сапфирон"
L["Saprish"] = "Сарпиш"
L["Sathel the Accursed"] = "Сатель Злосчастный"
L["Saurolisk Bonenipper"] = "Завролиск-костеглод"
L["Savage Cave Beast"] = "Дикий пещерный зверь"
L["Savage Worg"] = "Взбешенный ворг"
L["Scaffolding"] = "Леса"
L["Scalebane Lieutenant"] = "Лейтенант чешуйчатых губителей"
L["Scaled Krolusk Rider"] = "Чешуйчатый наездник на кролуске"
L["Scaled Krolusk Tamer"] = "Бронированный укротитель кролусков"
L["Scare Beast"] = "Отпугивание"
L["Scarlet Cannoneer"] = "Канонир Алого ордена"
L["Scarlet Centurion"] = "Центурион Алого ордена"
L["Scarlet Defender"] = "Защитник Алого ордена"
L["Scarlet Evangelist"] = "Проповедница Алого ордена"
L["Scarlet Evoker"] = "Вызыватель Алого ордена"
L["Scarlet Fanatic"] = "Фанатик Алого ордена"
L["Scarlet Flamethrower"] = "Метатель пламени Алого ордена"
L["Scarlet Hall Guardian"] = "Страж зала Алого ордена"
L["Scarlet Halls"] = "Залы Алого ордена"
L["Scarlet Initiate"] = "Посвященный из Алого ордена"
L["Scarlet Judicator"] = "Заступник Алого ордена"
L["Scarlet Monastery"] = "Монастырь Алого ордена"
L["Scarlet Myrmidon"] = "Мирмидон из Алого ордена"
L["Scarlet Pupil"] = "Последователь Алого ордена"
L["Scarlet Purifier"] = "Освятитель Алого ордена"
L["Scarlet Scholar"] = "Книжник Алого ордена"
L["Scarlet Scourge Hewer"] = "Рассекатель Алого ордена"
L["Scarlet Treasurer"] = "Хранитель сокровищ Алого ордена"
L["Scarlet Zealot"] = "Ревнитель из Алого ордена"
L["scarletHallsShortName"] = "ЗАО"
L["scarletMonasteryShortName"] = "МАО"
L["Scavenging Leaper"] = "Прыгун-падальшик"
L["Scholomance"] = "Некроситет"
L["Scholomance Acolyte"] = "Послушник из Некроситета"
L["Scholomance Neophyte"] = "Неофит из Некроситета"
L["scholomanceShortName"] = "Некро"
L["Scorchling"] = "Сполох"
L["Scourge Brute"] = "Громила Плети"
L["Scourge Hulk"] = "Увалень из Плети"
L["Scourge Reanimator"] = "Воскреситель из Плети"
L["Scrap Block"] = "Блок металла"
L["Scrapbone Bully"] = "Бугай из племени Хламоедов"
L["Scrapbone Grinder"] = "Перемалыватель из племени Хламоедов"
L["Scrapbone Grunter"] = "Ворчун из племени Хламоедов"
L["Scrapbone Shaman"] = "Шаман из племени Хламоедов"
L["Scrapbone Trashtosser"] = "Хламометатель из племени Хламоедов"
L["Scraphound"] = "Утильхаунд"
L["Scrimshaw Enforcer"] = "Вышибала из банды Резчиков"
L["Scrimshaw Gutter"] = "Кишкодер из банды Резчиков"
L["Seacursed Mistmender"] = "Проклятая морем целительница туманов"
L["Seacursed Slaver"] = "Проклятый морем рабовладелец"
L["Seacursed Soulkeeper"] = "Проклятый морем тюремщик душ"
L["Seacursed Swiftblade"] = "Проклятый морем молниеносный клинок"
L["Seasonal Affix:"] = "Сезонный аффикс:"
L["Seaspray Crab"] = "Морской краб"
L["Seat of the Archon"] = "Престол Архонта"
L["Seat of the Triumvirate"] = "Престол Триумвирата"
L["Seat of the Triumvirate Sublevel"] = "Престол Триумвирата"
L["seatOfTheTriumvirateShortName"] = "ПТ"
L["Select all"] = "Выбрать все"
L["Select the dungeon level"] = "Выбрать уровень подземелья"
L["Sending: %.1f"] = "Отправка: %.1f"
L["Seneschal M'bara"] = "Сенешаль М'бара"
L["Sentient Oil"] = "Разумное масло"
L["Sentinel Talondras"] = "Часовая Талондрас"
L["Sentry Stagshell"] = "Оленепанцирник-дозорный"
L["Separation Assistant"] = "Ассистент-расчленитель"
L["Sergeant Bainbridge"] = "Сержант Бейнбридж"
L["Sergeant Shaynemail"] = "Сержант Шейнмейл"
L["Serpentrix"] = "Змеикс"
L["Servant of Asaad"] = "Слуга Асаада"
L["Set Target Marker"] = "Установить метку на цель"
L["Settings"] = "Настройки"
L["Sewer Gate"] = "Сточная калитка"
L["Sewer Vicejaw"] = "Сточный злобнокус"
L["Sha of Doubt"] = "Ша Сомнения"
L["Sha of Violence"] = "Ша Жестокости"
L["Shackle Undead"] = "Сковывание нежити"
L["Shackled Soul"] = "Скованная душа"
L["Shade of Medivh"] = "Тень Медива"
L["Shade of Naxxramas"] = "Тень Наксрамаса"
L["Shade of Xavius"] = "Тень Ксавия"
L["Shado-Pan Ambusher"] = "Душитель Шадо-Пан"
L["Shado-Pan Disciple"] = "Послушник Шадо-Пан"
L["Shado-Pan Monastery"] = "Монастырь Шадо-Пан"
L["Shado-Pan Stormbringer"] = "Вестник шторма Шадо-Пан"
L["Shado-Pan Warden"] = "Страж Шадо-Пан"
L["shadoPanMonasteryShortName"] = "МШП"
L["Shadow Hunter"] = "Темная охотница"
L["Shadow Mistress"] = "Владычица теней"
L["Shadow of Zul"] = "Тень Зула"
L["Shadow Stalker"] = "Темный хищник"
L["Shadowblade Stalker"] = "Крадущийся теневой клинок"
L["Shadow-Borne Champion"] = "Тенеликий защитник"
L["Shadow-Borne Warrior"] = "Тенеликий воин"
L["Shadow-Borne Witch Doctor"] = "Тенеликий колдун"
L["Shadowguard Champion"] = "Защитник из Темной Стражи"
L["Shadowguard Conjurer"] = "Кудесник из Темной Стражи"
L["Shadowguard Riftstalker"] = "Охотник из Темной Стражи"
L["Shadowguard Subjugator"] = "Подчинитель из Темной Стражи"
L["Shadowguard Trickster"] = "Ловкач из Темной Стражи"
L["Shadowguard Voidbender"] = "Заклинатель Бездны из Темной Стражи"
L["Shadowlands"] = "Темные земли"
L["Shadowlands Season 4"] = "Темные земли 4-й сезон"
L["Shadowmoon Bone-Mender"] = "Подчинитель костей из клана Призрачной Луны"
L["Shadowmoon Dominator"] = "Поработитель из клана Призрачной Луны"
L["Shadowmoon Enslaver"] = "Поработитель из клана Призрачной Луны"
L["Shadowmoon Exhumer"] = "Извлекатель душ из клана Призрачной Луны"
L["Shadowmoon Loyalist"] = "Верная служительница из клана Призрачной Луны"
L["ShadowmoonBurialGrounds"] = "Некрополь Призрачной Луны"
L["shadowmoonShortName"] = "НПЛ"
L["Shady Dealer"] = "Сомнительный делец"
L["Shalebiter"] = "Сланцеед"
L["Shambling Arbalest"] = "Шаркающий арбалетчик"
L["Shambling Infester"] = "Неуклюжий заразитель"
L["Shard of Halkias"] = "Фрагмент Халкиаса"
L["Share"] = "Поделиться"
L["Share the preset with your party members"] = "Поделиться маршрутом с членами Вашей группы"
L["Sha-Touched Guardian"] = "Пораженный ша страж"
L["Shattered Visage"] = "Расколотое видение"
L["Shieldbearer of Zul"] = "Щитоносец из армии Зула"
L["Shift-Click to delete all presets for this dungeon"] = "[Shift+ЛКМ] - удалить все маршруты для этого подземелья"
L["Shikaar Ranger"] = "Шикаарский следопыт"
L["Shiprat"] = "Корабельная крыса"
L["Shock Bot"] = "Шоковый бот"
L["Shortcut"] = "Короткий путь"
L["Shreddinator 3000"] = "Крошшенатор-3000"
L["Shrieking Terror"] = "Визжащий ужас"
L["Shrieking Whelp"] = "Кричащий дракончик"
L["Shrine of the Storm"] = "Святилище Штормов"
L["Shrine of the Storm Sublevel"] = "Святилище Штормов (нижний уровень)"
L["Shrine Templar"] = "Храмовник из Святилища Штормов"
L["shrineGraveyardNote1"] = "Открывается после победы над Лордом Штормсонгом"
L["shrineOfTheStormsShortName"] = "СШ"
L["Shroud Hound"] = "Туманная гончая"
L["Shrouded"] = "Маскировка"
L["Shrouded Fang"] = "Скрытный убийца"
L["Shuffling Corpse"] = "Шаркающий труп"
L["Shuffling Horror"] = "Слоняющийся ужас"
L["Siege of Boralus"] = "Осада Боралуса"
L["Siege of Boralus (Upstairs)"] = "Осада Боралуса (верхний уровень)"
L["Siege of Boralus Sublevel"] = "Осада Боралуса"
L["Siege of Niuzao Temple"] = "Осада храма Нюцзао"
L["siegeDuplicateNote"] = "Г39 можно дублировать, если в этой области есть локальное задание"
L["siegeGraveyardNote1"] = "Открывается после победы над Сержантом Бейнбриджем / Головорезом Краснокрюком"
L["siegeGraveyardNote2"] = "Открывается после победы над Жутким капитаном Локвудом"
L["siegeGraveyardNote3"] = "Открывается после победы над Хадал Черная Бездна"
L["siegeGutterNote2"] =
  "Г72 Gutters можно использовать для вражеских войск, нанося им урон и вызывая мгновенное убийство у пехотинцев рядом с ними"
L["siegeGuttersNote"] =
  "Заметка про Кишкодеров:%sПехотинцы моментально убивают Кишкодеров, как только игрок приближается к ним. Если они умирают, не получая урона от группы, они не дают очков сил врага"
L["Siegemaster Olugar"] = "Осадный мастер Олугар"
L["Siegemaster Rokra"] = "Осадный мастер Рокра"
L["siegeOfBoralusShortName"] = "ОБ"
L["siegeOfNiuzaoTempleShortName"] = "ОХН"
L["Sik'thik Amber-Weaver"] = "Сик'тик - янтарный прядильщик"
L["Sik'thik Amberwing"] = "Сик'тик Желтокрыл"
L["Sik'thik Battle-Mender"] = "Сик'тик - боевой лекарь"
L["Sik'thik Bladedancer"] = "Сик'тик - танцующий с клинками"
L["Sik'thik Builder"] = "Сик'тик-строитель"
L["Sik'thik Demolisher"] = "Сик'тик-разрушитель"
L["Sik'thik Engineer"] = "Сик'тик-инженер"
L["Sik'thik Guardian"] = "Сик'тик-стражник"
L["Sik'thik Soldier"] = "Сик'тик-солдат"
L["Sik'thik Swarmer"] = "Сик'тик-роевик"
L["Sik'thik Vanguard"] = "Сик'тик - боец авангарда"
L["Sik'thik Warden"] = "Сик'тик-страж"
L["Sik'thik Warrior"] = "Сик'тик-воин"
L["Silence"] = "Немота"
L["Silt Crab"] = "Ильный краб"
L["Sinstone Statue"] = "Статуя Камня грехов"
L["Sir Braunpyke"] = "Сэр Браунпайк"
L["Sir Zeliek"] = "Сэр Зелиек"
L["Sister Briar"] = "Сестра Брайар"
L["Sister Malady"] = "Сестра Маладия"
L["Sister Solena"] = "Сестра Солена"
L["Sjonnir The Ironshaper"] = "Сьоннир Литейщик"
L["Skadi the Ruthless"] = "Скади Безжалостный"
L["Skardyn Monstrosity"] = "Скардин-чудовище"
L["Skarmorak"] = "Скарморак"
L["Skarvald the Constructor"] = "Скарвальд Строитель"
L["Skeletal Hound"] = "Костяная гончая"
L["Skeletal Hunting Raptor"] = "Охотничий ящер"
L["Skeletal Marauder"] = "Скелет-мародер"
L["Skeletal Monstrosity"] = "Костяное чудовище"
L["Skeletal Raptor"] = "Скелет-ящер"
L["Skeletal Smith"] = "Скелет-кузнец"
L["Skeletal Usher"] = "Гниющий билетер"
L["Skeletal Waiter"] = "Скелет-официант"
L["Skeletal Warrior"] = "Скелет-воин"
L["Skittering Assistant"] = "Быстролап-помощник"
L["Skittering Crawler"] = "Прыгучий паук"
L["Skittering Darkness"] = "Снующая тьма"
L["Skittering Swarmer"] = "Быстролапый членистоног"
L["Skittish"] = "Упрямый"
L["Skjal"] = "Скьял"
L["Skrog Tidestomper"] = "Скрог-волнодав"
L["Skrog Wavecrasher"] = "Скрог-волнолом"
L["Skulking Gutstabber"] = "Затаившийся брюхокол"
L["Skulking Zealot"] = "Затаившийся фанатик"
L["Skulloc"] = "Черепон"
L["Skycap'n Kragg"] = "Небесный капитан Крагг"
L["Skyfall Star"] = "Упавшая звезда"
L["Skylord Tovra"] = "Повелительница небес Товра"
L["Slad'ran"] = "Слад'ран"
L["Slag"] = "Шлак"
L["Sleep Walk"] = "Лунатизм"
L["Sleepy Hozen Brawler"] = "Сонный хозен-буян"
L["Slime Elemental"] = "Жижементаль"
L["Slime Tentacle"] = "Склизкое щупальце"
L["Slimy Morsel"] = "Осклизлое лакомство"
L["Slithering Ooze"] = "Ползучая слизь"
L["Slow"] = "Замедление"
L["Sludge"] = "Жижа"
L["Sludge Belcher"] = "Изрыгатель слизи"
L["Smashspite the Hateful"] = "Хмуродроб Лютый"
L["Smuggled Creature"] = "Контрабандное существо"
L["Snarling Dockhound"] = "Злобный портовый пес"
L["So' Cartel Assassin"] = "Убийца из картеля Со"
L["So'azmi"] = "Со'азми"
L["Sodden Hozen Brawler"] = "Отупевший хозен-буян"
L["Soggodon the Breaker"] = "Соггодон Ломатель"
L["Soggy Shiprat"] = "Мокрая корабельная крыса"
L["So'leah"] = "Со'лея"
L["Solsten"] = "Солстен"
L["Son of Hakkar"] = "Сын Хаккара"
L["Sootsnout"] = "Сажеморд <Кровожадный>"
L["Soul Essence"] = "Субстанция души"
L["Soulbound Goliath"] = "Оживленный голиаф"
L["Soulforged Bonereaver"] = "Усиленный душами похититель костей"
L["Soulharvester Duuren"] = "Жнец душ Даарен"
L["Soulharvester Galtmaa"] = "Жнец душ Галтмаа"
L["Soulharvester Mandakh"] = "Жнец душ Мандакх"
L["Soulharvester Tumen"] = "Жнец душ Тумен"
L["Soul-Scribe"] = "Переписчица душ"
L["Soul-Torn Champion"] = "Лишенный души защитник"
L["Spare Parts"] = "Запасные части"
L["Spark Channeler"] = "Искротворец"
L["Speaker Brokk"] = "Глашатай Брокк"
L["Speaker Dorlita"] = "Глашатай Дорлита"
L["Speaker Shadowcrown"] = "Проповедница Темная Корона"
L["Spectral Apprentice"] = "Призрачный подмастерье"
L["Spectral Attendant"] = "Призрачный смотритель"
L["Spectral Beastmaster"] = "Призрачная повелительница животных"
L["Spectral Berserker"] = "Призрачный берсерк"
L["Spectral Brute"] = "Призрачный громила"
L["Spectral Charger"] = "Призрачный конь"
L["Spectral Headhunter"] = "Призрачный охотник за головами"
L["Spectral Hex Priest"] = "Призрачный жрец-проклинатель"
L["Spectral Invoker"] = "Алгет'арская заклинательница"
L["Spectral Journeyman"] = "Призрачный работник"
L["Spectral Patron"] = "Призрачный зритель"
L["Spectral Retainer"] = "Призрачный эконом"
L["Spectral Sentry"] = "Призрачный стражник"
L["Spectral Stable Hand"] = "Призрачный помощник смотрителя стойл"
L["Spectral Valet"] = "Призрачный служитель"
L["Spectral Witch Doctor"] = "Призрачная знахарка"
L["Spellbound Battleaxe"] = "Зачарованный боевой топор"
L["Spellbound Scepter"] = "Зачарованный скипетр"
L["Spider"] = "Паук"
L["Spider Tank"] = "Танк-паук"
L["Spinemaw Acidgullet"] = "Иглобрюх-кислотник"
L["Spinemaw Gorger"] = "Иглобрюх-поглотитель"
L["Spinemaw Larva"] = "Личинка иглобрюха"
L["Spinemaw Reaver"] = "Иглобрюх-разоритель"
L["Spinemaw Staghorn"] = "Иглобрюх-рогач"
L["Spires of Ascension"] = "Шпили Перерождения"
L["spiresOfAscensionShortName"] = "ШП"
L["Spirit of Vengeance"] = "Дух отмщения"
L["Spiteful"] = "Злопамятный"
L["Spitting Cobra"] = "Плюющаяся кобра"
L["Splotch"] = "Пятно"
L["Sporecaller Zancha"] = "Заклинатель спор Занча"
L["Spriggan Barkbinder"] = "Спригган - заклинатель коры"
L["Spriggan Mendbender"] = "Спригган - подчинитель разума"
L["Spurlok, Timesworn Sentinel"] = "Шпорлок Часовой Времени"
L["Squallbringer Cyraz"] = "Вестник бури Сайраз"
L["Stalagg"] = "Сталагг"
L["Start or join the current |cFF00FF00Live Session|r"] = "Начните или присоединитесь к текущей |cFF00FF00'Прямой трансляции'|r"
L["Starved Crawler"] = "Изголодавшийся ползун"
L["Starving Hound"] = "Голодная гончая"
L["Static-charged Dervish"] = "Наэлектризованный дервиш"
L["Stealthling"] = "Крадущийся паучок"
L["Steelbreaker"] = "Сталелом"
L["Steeljaw Grizzly"] = "Зубастый гризли"
L["Steward"] = "Распорядитель"
L["Stinging Parasite"] = "Жалящий паразит"
L["Stinging Swarm"] = "Жалящая мошкара"
L["Stinkbreath"] = "Мерзорот"
L["Stitched Colossus"] = "Сшитый колосс"
L["Stitched Giant"] = "Лоскутный изрыгатель"
L["Stitched Vanguard"] = "Сшитый боец авангарда"
L["Stitchflesh's Creation"] = "Создание Трупошва"
L["Stitching Assistant"] = "Ассистент-сшиватель"
L["stolenPowerDescription"] = "Может использоваться разбойниками, жрецами и инженерами Каз Алгар (25)"
L["Stoneborn Eviscerator"] = "Камнерожденная-потрошительница"
L["Stoneborn Reaver"] = "Камнерожденный-разоритель"
L["Stoneborn Slasher"] = "Камнерожденный-рассекатель"
L["Stoneclaw Grubmaster"] = "Камнерукий повелитель личинок"
L["Stoneclaw Hunter"] = "Камнерукий охотник"
L["Stonefiend Anklebiter"] = "Мелкий камнебес"
L["Stonefury"] = "Каменный яростень"
L["Stoneskin Gargoyle"] = "Каменная горгулья"
L["Stonevault Geomancer"] = "Геомант из племени Каменного Свода"
L["stoneVaultShortName"] = "КС"
L["Stonewall Gargon"] = "Гаргон-защитник"
L["Storm Drake"] = "Штормовой дракон"
L["Storm Tempered Keeper"] = "Закаленный бурями хранитель"
L["Storm Warrior"] = "Воин бури"
L["Stormcaller Arynga"] = "Призывательница бури Аринга"
L["Stormcaller Boroo"] = "Призыватель бури Бороо"
L["Stormcaller Brundir"] = "Буревестник Брундир"
L["Stormcaller Solongo"] = "Призыватель бури Солонго"
L["Stormcaller Zarii"] = "Призыватель бури Зарии"
L["Stormflurry Totem"] = "Тотем грозового шквала"
L["Stormforged Construct"] = "Создание клана Закаленных Бурей"
L["Stormforged Giant"] = "Великан из клана Закаленных Бурей"
L["Stormforged Guardian"] = "Бурекованый страж"
L["Stormforged Lieutenant"] = "Лейтенант клана Закаленных Бурей"
L["Stormforged Mender"] = "Лекарь клана Закаленных Бурей"
L["Stormforged Runeshaper"] = "Рунодел клана Закаленных Бурей"
L["Stormforged Sentinel"] = "Часовой клана Закаленных Бурей"
L["Stormforged Tactician"] = "Тактик клана Закаленных Бурей"
L["Stormfury Revenant"] = "Загробник ярости бурь"
L["Stormguard Gorren"] = "Бурестраж Горрен"
L["Storming"] = "Бушующий"
L["Storming Vortex"] = "Бушующий вихрь"
L["Stormrider Vokmar"] = "Странник бурь Вокмар"
L["Storm's End"] = "Предел шторма"
L["Stormstout Brewery"] = "Хмелеварня Буйных Портеров"
L["Stormstout Brewhall"] = "Хмелеваренный зал Буйных Портеров"
L["stormstoutBreweryShortName"] = "ХБП"
L["Stormsurge Totem"] = "Тотем энергии бури"
L["Stormwake Hydra"] = "Потревоженная бурей гидра"
L["Stout Brew Alemental"] = "Тучный бражный хмелементаль"
L["Stranglevine Lasher"] = "Плеточник-душитель"
L["Stratholme City"] = "Стратхольм"
L["streetBazaarEventNote1"] = "Первая волна <Базарное событие>"
L["streetBazaarEventNote2"] = "Вторая волна <Базарное событие>"
L["streetBazaarEventNote3"] = "Третья волна <Базарное событие>"
L["streetOasisBossNote1"] = "Первая волна <Оазис Мизы>"
L["streetOasisBossNote2"] = "Вторая волна <Оазис Мизы>"
L["Strider Tonk"] = "Танк-долгоног"
L["Strife"] = "Раздор"
L["Striker Ga'dok"] = "Боец Га'док"
L["Stun"] = "Оглушение"
L["Subjugated Soul"] = "Порабощенная душа"
L["Subterranean Proto-Dragon"] = "Подземный протодракон"
L["Sudsy Brew Alemental"] = "Пенистый бражный хмелементаль"
L["Sunken Denizen"] = "Утонувший обитатель глубин"
L["Support MDT on Patreon"] = "Поддержите MDT на Patreon"
L["Support Officer"] = "Офицер поддержки"
L["Sureki Conscript"] = "Суреки-новобранец"
L["Sureki Militant"] = "Суреки-ополченец"
L["Sureki Silkbinder"] = "Суреки – повелитель шелка"
L["Sureki Unnaturaler"] = "Суреки-неестественник"
L["Sureki Venomblade"] = "Суреки-ядошип"
L["Sureki Webmage"] = "Сурекийский маг паутины"
L["Surgeon Stitchflesh"] = "Хирург Трупошов"
L["Surgical Assistant"] = "Ассистент хирурга"
L["Svala Sorrowgrave"] = "Свала Вечноскорбящая"
L["Swampface"] = "Торфоморд"
L["Taah'bat"] = "Таа'бат"
L["Taener Duelmal"] = "Тейнер Дуэльмаль"
L["Tainted Sentry"] = "Опороченный часовой"
L["Taintheart Deadeye"] = "Скверносерд-стрелок"
L["Taintheart Stalker"] = "Скверносерд-лазутчик"
L["Taintheart Summoner"] = "Скверносерд-призыватель"
L["Talixae Flamewreath"] = "Таликса Пламя Гнева"
L["Tamed Phoenix"] = "Прирученный феникс"
L["Tamed Ruinstalker"] = "Прирученная прыгуана-мародер"
L["Tank Buster MK1"] = "Танкоборец-1"
L["Taran Zhu"] = "Тажань Чжу"
L["Tarasek Delver"] = "Тарасекк-расхититель <Расколотое Пламя>"
L["Tarspitter Lurker"] = "Смолоплюй-ползун"
L["Tarspitter Slug"] = "Смолоплюй-слизень"
L["Taskmaster Askari"] = "Надсмотрщик Аскари"
L["Taste Tester"] = "Дегустатор"
L["Tasting Room Attendant"] = "Работница дегустационной"
L["Taunt"] = "Провокация"
L["TazaveshFloor1"] = "Скрытый рынок"
L["TazaveshFloor2"] = "Большой Зверинец"
L["TazaveshFloor3"] = "Отделение ПОЧТ"
L["TazaveshFloor4"] = "Оазис Ау'мизы"
L["TazaveshFloor5"] = "Штормхейм"
L["TazaveshFloor6"] = "Хранилище Агграмара"
L["TazaveshFloor7"] = "Гавань Боралуса"
L["TazaveshFloor8"] = "Верхний город"
L["TazaveshLower"] = "Тазавеш: Улицы Чудес"
L["tazaveshLowerShortName"] = "ТУЧ"
L["TazaveshUpper"] = "Тазавеш: Гамбит Со'леи"
L["tazaveshUpperShortName"] = "ТГС"
L["tdBuffGateNote"] =
  "Шанс удержать союзника, который может дать Вам бафф, зависящий от его расы:%sОрк/дворф: 10%% сила/ловкость%sНежить/человек: 10%% выносливость%sТаурен/дреней: 1%% здоровье+мана за 10 сек.%sЭльф крови/гном: 10%% интеллект"
L["tdGraveyardNote1"] = "Открывается после победы над Королевой песков"
L["tdGraveyardNote2"] = "Открывается после победы над Джесом Хаулисом"
L["tdGraveyardNote3"] = "Открывается после победы над Рыцарем-капитаном Валири"
L["tdHowlisNote"] = "Джес Хаулис откроет эту клетку и нападет на врагов внутри нее"
L["TDPrisonKeyText"] =
  "Выброшенный ключ от клетки%sВозможное место появления Выброшенного ключа от клетки%sОткрывает одну тюремную клетку"
L["tdWardenFightingNote"] = "Г23 сражается и в конечном итоге убьет Надзирателя блока 1"
L["Teeming"] = "Кишащий"
L["Teera"] = "Тиира"
L["Telash Greywing"] = "Телаш Серокрылый"
L["Tempest Channeler"] = "Чаротворица бури"
L["Temple Adept"] = "Служитель храма"
L["Temple Attendant"] = "Служитель храма"
L["Temple of Sethraliss"] = "Храм Сетралисс"
L["Temple of Sethraliss Sublevel"] = "Храм Сетралисс"
L["Temple of the Jade Serpent"] = "Храм Нефритовой Змеи"
L["templeEyeNote"] =
  "Глаз Сетралисс%sПринесите оба глаза к Черепу Сетралисс%sКаждый глаз, который был вставлен в череп, дает 12 очков сил врага"
L["templeGraveyardNote1"] = "Открывается после победы над Меректой"
L["templeGraveyardNote2"] = "Открывается после завершения события 'Череп Сетралисс'"
L["templeOfSethralisShortName"] = "ХС"
L["TempleOfTheJadeSerpent"] = "Храм Нефритовой Змеи"
L["templeOfTheJadeSerpentShortName"] = "ХНЗ"
L["Temporal Deviation"] = "Временное искажение"
L["Temporal Fusion"] = "Временное слияние"
L["Terrified Broker"] = "Напуганный брокер"
L["Territorial Bladebeak"] = "Агрессивный мечеклюв"
L["Territorial Eagle"] = "Агрессивный орел"
L["Test Subject"] = "Подопытная крыса"
L["Thaddius"] = "Таддиус"
L["Thalnos the Soulrender"] = "Талнос Воздаятель Душ"
L["Thane Korth'azz"] = "Тан Корт'азз"
L["The Antechamber of Ulduar"] = "Вестибюль Ульдуара"
L["The Arachnid Quarter"] = "Паучий квартал"
L["The Arcway"] = "Катакомбы Сурамара"
L["The Arcway Sublevel"] = "Катакомбы Сурамара"
L["The Balconies"] = "Балконы"
L["The Banquet Hall"] = "Пиршественный зал"
L["The Brig"] = "Гауптвахта"
L["The Brood Pit"] = "Родовая яма"
L["The Burning Cauldron"] = "Горячий котел"
L["The Candle King"] = "Свечной Король"
L["The Cellar"] = "Погреб"
L["The Coaglamation"] = "Сгустолиция"
L["The Construct Quarter"] = "Квартал Мерзости"
L["The Crimson Assembly Hall"] = "Алый зал собраний"
L["The Crybaby Hozen"] = "Хозен-плакса"
L["The Culling of Stratholme"] = "Очищение Стратхольма"
L["The Curator"] = "Смотритель"
L["The Darkness"] = "Тьма"
L["The Dawnbreaker"] = "Сияющий Рассвет"
L["The Drain"] = "Стоки"
L["The Emerald Archives"] = "Изумрудные архивы"
L["The Everbloom"] = "Вечное Цветение"
L["The Festering Sanctum"] = "Гниющее святилище"
L["The Gilded Gate"] = "Золоченые врата"
L["The Golden Beetle"] = "Золотой жук"
L["The Golden Serpent"] = "Золотой Змей"
L["The Grand Foyer"] = "Главный вестибюль"
L["The Grand Hall"] = "Главный зал"
L["The Great Wheel"] = "Водяное колесо"
L["The Grimewalker"] = "Грязеход"
L["The Guest Chambers"] = "Гостевые комнаты"
L["The High Gate"] = "Высокие врата"
L["The Hold"] = "Трюм"
L["The Inner Sanctum of Ulduar"] = "Внутреннее святилище Ульдуара"
L["The Jeweled Estate"] = "Самоцветный дворец"
L["The Lower Necropolis"] = "Нижний некрополь"
L["The Menagerie"] = "Зверинец"
L["The Military Quarter"] = "Военный квартал"
L["The Mind's Eye"] = "Око Разума"
L["The MOTHERLODE!!"] = "ЗОЛОТАЯ ЖИЛА!!!"
L["The MOTHERLODE!! Sublevel"] = "ЗОЛОТАЯ ЖИЛА!!!"
L["The Naglfar"] = '"Нагльфар"'
L["The Necrotic Wake"] = "Смертельная тризна"
L["The Nexus"] = "Нексус"
L["The Nodding Tiger"] = "Кивающий тигр"
L["The Oculus"] = "Окулус"
L["The Plague Quarter"] = "Чумной квартал"
L["The Platinum Pummeler"] = "Платиновый лупцеватор"
L["The preset will continuously synchronize between all party members participating in the Live Session"] =
  "Маршрут будет постоянно синхронизироваться между всеми членами группы, которые участвуют в 'Прямой трансляции'"
L["The Prison of Yogg-Saron"] = "Тюрьма Йогг-Сарона"
L["The Prophet Tharon'ja"] = "Пророк Тарон'джа"
L["The Raging Tempest"] = "Яростная буря"
L["The Raven's Crown"] = "Корона Ворона"
L["The Ravenscrypt"] = "Крипта Воронов"
L["The Reliquary"] = "Хранилище реликвий"
L["The Robodrome"] = "Рободром"
L["The Rookery"] = "Гнездовье"
L["The Rook's Host"] = "Гнездовье Черной Ладьи"
L["The Rupture"] = "Разлом"
L["The Sand Queen"] = "Королева песков"
L["The selected affixes are not the ones of the current week"] = "Выбранные аффиксы не соответствуют текущей неделе"
L["The selected dungeon level is below 10"] = "Выбранный уровень подземелья ниже 10"
L["The selected level will affect displayed npc health"] =
  "Выбранный уровень повлияет на отображаемое количество здоровья НИП"
L["The Servant's Quarters"] = "Комнаты слуг"
L["The Songbird Queen"] = "Королева певчих птиц"
L["The Spark of Imagination"] = "Искра Воображения"
L["The Stonevault"] = "Каменный Свод"
L["The Talking Fish"] = "Говорящая рыба"
L["The Tasting Room"] = "Дегустационная"
L["The Under Junk"] = "Барахлимб"
L["The Underrot"] = "Подгнилье"
L["The Underrot Sublevel"] = "Подгнилье"
L["The Upper Necropolis"] = "Верхний некрополь"
L["The Upper Study"] = "Верхний кабинет"
L["The Vault of Tyr"] = "Хранилище Тира"
L["The Vestibules of Drak'Tharon"] = "Вестибюли Драк'Тарона"
L["The Violet Hold"] = "Аметистовая крепость"
L["The Vortex Pinnacle"] = "Вершина Смерча"
L["The War Within Season 1"] = "Война Внутри 1-й сезон"
L["The War Within Season 2"] = "Война Внутри 2-й сезон"
L["The War Within Season 3"] = "Война Внутри 3-й сезон"
L["The Warden's Court"] = "Суд Стражей"
L["TheArcaneConservatory"] = "Магический консерваторий"
L["Theater of Pain"] = "Театр Боли"
L["theaterOfPain_miniBossNote"] = "В живых останется только один дуэлянт."
L["TheaterOfPainFloor1"] = "Театр Боли"
L["TheaterOfPainFloor2"] = "Зал Победы"
L["TheaterOfPainFloor3"] = "Алтари Агонии"
L["TheaterOfPainFloor4"] = "Верхний уровень кургана Резни"
L["TheaterOfPainFloor5"] = "Нижний уровень кургана Резни"
L["theaterOfPainShortName"] = "ТБ"
L["TheAzureVault"] = "Лазурное хранилище"
L["TheBurningCauldron"] = "Горячий котел"
L["TheHeadteachersEnclave"] = "Анклав директора"
L["TheNecroticWakeFloor1"] = "Смертельная тризна"
L["TheNecroticWakeFloor2"] = "Трупосшивальня"
L["TheNecroticWakeFloor3"] = '"Золрамус"'
L["TheNokhudOffensive"] = "Наступление клана Нокхуд"
L["ThePitch"] = "Площадка"
L["TheScrollkeepersSanctum"] = "Хранилище Хранителя свитков"
L["TheVaultOfTyr"] = "Хранилище Тира"
L["Thistle Acolyte"] = "Ведьма-послушница"
L["Thorim"] = "Торим"
L["Thornguard"] = "Шипастый страж"
L["Thrashbite the Scornful"] = "Долбогрыз Глумливый"
L["Throne of Ancient Conquerors"] = "Трон древних завоевателей"
L["Throne of Tides"] = "Трон Приливов"
L["throneOfTidesShortName"] = "ТП"
L["Thunderhead"] = "Громоголов"
L["Thundering"] = "Рокочущий"
L["Thunderlord Wrangler"] = "Укротитель из клана Громоборцев"
L["Ticking Time Bomb"] = "Тикающая часовая бомба"
L["Tidesage Enforcer"] = "Жрец моря - каратель"
L["Tidesage Initiate"] = "Ученик жрецов моря"
L["Tidesage Spiritualist"] = "Жрица моря - спиритуалистка"
L["Timecap'n Hooktail"] = "Хронокэп Крюкохвост"
L["Time-Displaced Trooper"] = "Затерянный во времени солдат"
L["Timeline Marauder"] = "Темпоральный мародер"
L["Time-Lost Aerobot"] = "Затерянный во времени аэробот"
L["Time-Lost Rocketeer"] = "Затерянный во времени ракетчик"
L["Time-Lost Smack-o-Tron"] = "Затерянный во времени хлестотрон"
L["Time-Lost Tidehunter"] = "Затерянный во времени волнолов"
L["Time-Lost Wakethrasher"] = "Затерянный во времени волношлеп"
L["Time-Lost Waveshaper"] = "Затерянный во времени заклинатель моря"
L["Timer"] = "Таймер"
L["Timestream Anomaly"] = "Аномалия временного потока"
L["Timestream Leech"] = "Пиявка из временного потока"
L["Tirathon Saltheril"] = "Тиратон Салтерил"
L["Tirnenn Villager"] = "Жительница Тирна Скитта"
L["Titanic Defense Turret"] = "Защитная турель титанов"
L["Titanium Siegebreaker"] = "Титановый осадник"
L["Titanium Thunderer"] = "Титановый грозоносец"
L["Titanium Vanguard"] = "Титановый воин"
L["T'lonja"] = "Т'лонджа"
L["To share a different preset while the live session is active simply navigate to the preferred preset and click the new 'Set to Live' Button next to the preset-dropdown"] =
  "Чтобы поделиться другим маршрутом во время активного сеанса прямой трансляции, просто перейдите к предпочитаемому маршруту и ​​нажмите новую кнопку 'Установить для прямой трансляции' рядом с раскрывающимся списком маршрутов"
L["tocNotes"] = "Инструмент для планирования и оптимизации прохождения подземелий М+"
L["togc"] = "Испытание крестоносца"
L["togcFloor1"] = "Колизей Серебряного Авангарда"
L["togcFloor2"] = "Ледяные глубины"
L["Toggle MDT"] = "Включить/выключить MDT"
L["Toggle Window"] = "Переключить окно"
L["Toiling Groundskeeper"] = "Работящий смотритель"
L["Tol Dagor"] = "Тол Дагор"
L["Tol Dagor Sublevel1"] = "Тол Дагор"
L["tolDagorShortName"] = "ТД"
L["Tomb Horror"] = "Могильный ужас"
L["Tomb Stalker"] = "Кладбищенский ловец"
L["Torchsnarl"] = "Пламескал"
L["Tormented Bloodseeker"] = "Истерзанный кровопийца"
L["Tormented Soul"] = "Замученная душа"
L["Total"] = "Всего"
L["Totem"] = "Тотем"
L["Toxic Lurker"] = "Токсичный скрытень"
L["Toxic Monstrosity"] = "Токсичное чудище"
L["Toxic Saurid"] = "Ядовитый саурид"
L["Tracker Zo'korss"] = "Следопыт Зо'корсс"
L["Train Depot"] = "Депо Мрачных Путей"
L["Training Grounds"] = "Тренировочная площадка"
L["Trash"] = "Мусор"
L["Tred'ova"] = "Тред'ова"
L["Treemouth"] = "Древорот"
L["Tribunal of the Ages"] = "Трибунал веков"
L["Trickclaw Mystic"] = "Хитроклык-мистик"
L["Tricktotem"] = "Лови-тотем"
L["Trilling Attendant"] = "Служитель-щелкун"
L["Trixie Tazer"] = "Трикси Искрожгучка"
L["Trollgore"] = "Кровотролль"
L["Trothak"] = "Тротак"
L["Troubled Soul"] = "Неспокойная душа"
L["Tunneling Ghoul"] = "Туннельный вурдалак"
L["Turbulent Squall"] = "Вихревой шквал"
L["Turn Evil"] = "Изгнание зла"
L["Turned Speaker"] = "Обращенный глашатай"
L["Twilight Adherent"] = "Сумеречный приверженец"
L["Twilight Apostle"] = "Сумеречный апостол"
L["Twilight Beguiler"] = "Сумеречный обманщик"
L["Twilight Brute"] = "Сумеречный громила"
L["Twilight Darkcaster"] = "Сумеречный черный маг"
L["Twilight Destroyer"] = "Сумеречный разрушитель"
L["Twilight Earthcaller"] = "Сумеречный заклинатель земли"
L["Twilight Enforcer"] = "Сумеречный головорез"
L["Twilight Flamerender"] = "Сумеречный пламеруб"
L["Twilight Frost Mage"] = "Сумеречный ледяной маг"
L["Twilight Grove"] = "Сумеречная роща"
L["Twilight Guardian"] = "Сумеречный страж"
L["Twilight Initiate"] = "Сумеречный посвященный"
L["Twilight Lavabender"] = "Сумеречный лавомант"
L["Twilight Overseer"] = "Сумеречный надзиратель"
L["Twilight Pyromancer"] = "Сумеречный пиромант"
L["Twilight Shadowblade"] = "Сумеречный душегуб"
L["Twilight Slayer"] = "Сумеречный убийца"
L["Twilight Warlock"] = "Чернокнижник Сумеречного Молота"
L["Twilight Worshipper"] = "Сумеречный верующий"
L["Twisted Abomination"] = "Искаженное поганище"
L["Tyr, the Infinite Keeper"] = "Тир, Хранитель Бесконечности"
L["Tyrannical"] = "Тиранический"
L["Tyr's Terrace"] = "Терраса Тира"
L["Tyr's Vanguard"] = "Боец авангарда Тира"
L["Ukhel Beastcaller"] = "Укхельский заклинатель зверей"
L["Ukhel Corruptor"] = "Укхельский осквернитель"
L["Ukhel Deathspeaker"] = "Укхельский вестник смерти"
L["Ukhel Willcrusher"] = "Укхельский сокрушитель воли"
L["Ularogg Cragshaper"] = "Уларогг Скаломол"
L["Uldaman: Legacy of Tyr"] = "Ульдаман: Наследие Тира"
L["UldamanLegacyOfTyr"] = "Ульдаман: Наследие Тира"
L["uldamanShortName"] = "УНТ"
L["Ulduar"] = "Ульдуар"
L["Umbral War-Adept"] = "Темный адепт войны"
L["Umbrelskul"] = "Мраскул"
L["Unbound Abomination"] = "Освобожденное чудовище"
L["Unbound Ethereal"] = "Освобожденный эфириал"
L["Unbound Firestorm"] = "Необузданный огненный шторм"
L["Uncategorized"] = "Без категории"
L["Undead"] = "Нежить"
L["Undercrawler"] = "Подземный ползун"
L["Underrot Tick"] = "Клещ из Подгнилья"
L["underrotMatronNote"] = "Матрона 4+5 может появляться как на левой, так и на правой платформе"
L["underrotShortName"] = "ГНИЛЬЕ"
L["underrotSkipNote"] = "Короткий путь%sДоступен после убийства Заклинателя спор Занча"
L["underrotVoidNote"] =
  "Порождения Меченных Бездной посланниц 2 и 3 случайны.%sТолько одна из них будет присутствовать одновременно."
L["Understone Demolisher"] = "Разрушитель из Подкаменного разлома"
L["Understone Drudge"] = "Рабочий из Подкаменного разлома"
L["Understone Drummer"] = "Барабанщик из Подкаменного разлома"
L["Undo"] = "Отменить"
L["undoDrawing"] = "Отменить рисунок"
L["Undying Servant"] = "Неупокоенный слуга"
L["Undying Stonefiend"] = "Неумирающий камнебес"
L["Unholy Axe"] = "Нечестивый топор"
L["Unholy Staff"] = "Нечестивый посох"
L["Unholy Swords"] = "Нечестивые Мечи"
L["Unknown"] = "Неизвестный"
L["Unrelenting Construct"] = "Жестокое создание"
L["Unruly Ogron"] = "Непокорный огрон"
L["Unruly Patron"] = "Буйный завсегдатай"
L["Unruly Stormrook"] = "Непослушный буреклюв"
L["Unruly Textbook"] = "Хаотичный учебник"
L["Unstable Amalgamation"] = "Нестабильное слияние"
L["Unstable Canister"] = "Нестабильная канистра"
L["Unstable Corruption"] = "Нестабильная порча"
L["Unstable Curator"] = "Нестабильный смотритель"
L["Unstable Darkness"] = "Нестабильная тьма"
L["Unstable Larva"] = "Нестабильная личинка"
L["Unstable Squall"] = "Нестабильный шквал"
L["Unstable Test Subject"] = "Нестабильный подопытный объект"
L["Unyielding Constrictor"] = "Непреклонный душитель"
L["Unyielding Contender"] = "Непреклонный соперник"
L["Unyielding Garrison"] = "Стойкий гарнизон"
L["Update"] = "Обновить"
L["updateNote"] = "Пожалуйста, обновите аддон до последней версии или удалите его."
L["Upper Broken Stair"] = "Верх разрушенной лестницы"
L["Upper Library"] = "Верхний ярус библиотеки"
L["Upper Livery Stables"] = "Стойла"
L["Upper Pinnacle"] = "Верхняя вершина"
L["UpperChamber"] = "Верхний зал"
L["Upstairs"] = "Верхний уровень"
L["Urg'roth, Breaker of Heroes"] = "Ург'рот Сокрушитель Героев"
L["Urh Relic"] = "Реликвия Урт"
L["Use /mdt reset to restore the default position and scale of MDT."] =
  "Используйте команду /mdt reset, чтобы восстановить положение и масштаб MDT по умолчанию."
L["Use as a starting point:"] = "Использовать как начальную точку:"
L["Use forces count"] = "Использовать количество сил противников"
L["Utgarde Keep"] = "Крепость Утгард"
L["Utgarde Pinnacle"] = "Вершина Утгард"
L["Valarjar Aspirant"] = "Валарьяр-претендентка"
L["Valarjar Champion"] = "Валарьяр-защитник"
L["Valarjar Falconer"] = "Валарьяр-сокольница"
L["Valarjar Marksman"] = "Валарьяр-лучница"
L["Valarjar Mystic"] = "Валарьяр-мистик"
L["Valarjar Purifier"] = "Валарьяр-очиститель"
L["Valarjar Runecarver"] = "Валарьяр - резчик рун"
L["Valarjar Shieldmaiden"] = "Валарьяр - дева щита"
L["Valarjar Thundercaller"] = "Валарьяр - призыватель молний"
L["Valarjar Trapper"] = "Валарьяр-зверолов"
L["Valiona"] = "Валиона"
L["Valow, Timesworn Keeper"] = "Валов Хранитель Времени"
L["Varos Cloudstrider"] = "Варос Заоблачный Странник"
L["Vault Guard"] = "Страж хранилища"
L["Vault of the Betrayer"] = "Каземат Предателя"
L["Vault of the Wardens"] = "Казематы Стражей"
L["Vault of the Wardens Sublevel"] = "Казематы Стражей"
L["Vault Purifier"] = "Очиститель из хранилища"
L["vaultOfTheWardensShortName"] = "КС"
L["Vaults of Kings Past"] = "Королевское хранилище"
L["Venom Stalker"] = "Ядовитый паук-скакун"
L["Venomfang"] = "Ядовитый паук"
L["Venomous Ophidian"] = "Ядовитый змей"
L["Venomous Sniper"] = "Стрелок-отравитель"
L["Venomous Sniper Captain"] = "Капитан снайперов-отравителей"
L["Vent Stalker"] = "Ловчий из люка"
L["Ventunax"] = "Вентунакс"
L["Venture Co. Alchemist"] = "Алхимик Торговой компании"
L["Venture Co. Architect"] = "Архитектор Торговой компании"
L["Venture Co. Contractor"] = "Подрядчик Торговой компании"
L["Venture Co. Diver"] = "Ныряльщик Торговой компании"
L["Venture Co. Earthshaper"] = "Геомант Торговой компании"
L["Venture Co. Electrician"] = "Электрик Торговой компании"
L["Venture Co. Honey Harvester"] = "Сборщица меда из Торговой компании"
L["Venture Co. Longshoreman"] = "Докер Торговой компании"
L["Venture Co. Mastermind"] = "Гений Торговой компании"
L["Venture Co. Patron"] = "Посетитель из Торговой компании"
L["Venture Co. Pyromaniac"] = "Пироман из Торговой компании"
L["Venture Co. Skyscorcher"] = "Огнелетчица Торговой компании"
L["Venture Co. Surveyor"] = "Геодезист Торговой компании"
L["Venture Co. War Machine"] = "Боевая машина Торговой компании"
L["Venza Goldfuse"] = "Венца Голдаплавс"
L["Vermin Trapper"] = "Крысолов"
L["Vestige of Doubt"] = "Частица сомнений"
L["Vestige of Hatred"] = "Крупица Ненависти"
L["Veteran Sparkcaster"] = "Опытный заклинатель искр"
L["Vexamus"] = "Вексам"
L["Viceroy Nezhar"] = "Наместник Незжар"
L["Vicious Basilisk"] = "Злобный василиск"
L["Vicious Gargon"] = "Свирепый гаргон"
L["Vicious Hyena"] = "Злобная гиена"
L["Vicious Manafang"] = "Злобный зачарованный паук"
L["Vicious Mandragora"] = "Злобная мандрагора"
L["Vicious Snap Dragon"] = "Злобный морской варан"
L["Vigilant Duskwatch"] = "Бдительный часовой из Сумеречной стражи"
L["Vigilant Watchman"] = "Бдительный часовой"
L["Vile Lasher"] = "Мерзкий плеточник"
L["Vile Rothexer"] = "Злобная заклинательница гнили"
L["Vilebark Walker"] = "Ходячий гиблодрев"
L["Vileshard Chunk"] = "Злобнозем-глыба"
L["Vileshard Crawler"] = "Злобнозем-ползун"
L["Vileshard Hulk"] = "Злобнозем-исполин"
L["Vilethorn Blossom"] = "Цветущий злошип"
L["Viletongue Belcher"] = "Скверноуст-изрыгатель"
L["Viqgoth"] = "Вик'Гот"
L["Viq'Goth"] = "Вик'Гот"
L["Virtuous Lady"] = "Благочестивая дама"
L["Virulax Blightweaver"] = "Вирулакс Заклинатель Чумы"
L["Viz'aduum the Watcher"] = "Виз'адуум Всевидящий"
L["Vizier Jin'bak"] = "Визирь Цзинь'бак"
L["Void Ascendant"] = "Перерожденная служительница Бездны"
L["Void Bound Despoiler"] = "Зачарованный Бездной расхититель"
L["Void Bound Howler"] = "Подчиненный Бездной завыватель"
L["Void Discharge"] = "Выброс Бездны"
L["Void Flayer"] = "Щупальце-живодер Бездны"
L["Void Fragment"] = "Фрагмент Бездны"
L["Void Spawn"] = "Дитя Бездны"
L["Void Speaker Eirich"] = "Вестник Бездны Эйрих"
L["Void Touched Elemental"] = "Меченый Бездной элементаль"
L["Void-Cursed Crusher"] = "Проклятый Бездной крушитель"
L["Voidrider"] = "Странник Бездны"
L["Voidstone Awakened"] = "Пробудившийся Бездны"
L["Voidstone Monstrosity"] = "Чудище камня Бездны"
L["Void-Touched Emissary"] = "Меченная Бездной посланница"
L["Voidweaver Mal'thir"] = "Мал'тир - маг Бездны"
L["Volatile Energy"] = "Нестабильная энергия"
L["Volatile Memory"] = "Нестабильное воспоминание"
L["Volatile Sapling"] = "Нестабильный побег"
L["Volcanic"] = "Вулканический"
L["Vol'kaal"] = "Вол'каал"
L["Volkhan"] = "Волхан"
L["Vol'zith the Whisperer"] = "Вол'зит Шепчущая"
L["Voracious Gorger"] = "Ненасытный поглотитель"
L["VortexPinnacle"] = "Вершина Смерча"
L["vortexPinnacleShortName"] = "ВС"
L["Vx"] = "Вкс <Клык королевы>"
L["Vy Relic"] = "Реликвия Ви"
L["Walk of the Makers"] = "Галерея Творцов"
L["Wandering Candle"] = "Блуждающая свеча"
L["Wandering Pulsar"] = "Блуждающий пульсар"
L["Wandering Shellback"] = "Бродячий панцирник"
L["Wanton Sapper"] = "Бесшабашная подрывница"
L["War Lynx"] = "Боевая рысь"
L["War Ohuna"] = "Боевой охун"
L["Warlord Parjesh"] = "Полководец Паржеш"
L["Warlord Sargha"] = "Полководец Сарга"
L["Warning"] = "Предупреждение"
L["WARNING_OLD_DUNGEON_IMPORT"] =
  "Этот маршрут ведёт из подземелья, которое недоступно в текущей версии MDT. Установите MDT Legacy, чтобы импортировать этот маршрут."
L["Warp Shade"] = "Искаженная тень"
L["Warp Stalker"] = "Прыгуана"
L["Waste Pipes"] = "Барахлопровод"
L["Waste Processing Unit"] = "Переработчик отходов"
L["Wastelander Farstalker"] = "Пустынник-дальноход"
L["Wastelander Pactspeaker"] = "Пустынница – хранительница пакта"
L["Wastelander Phaseblade"] = "Пустынник – фазовый клинок"
L["Wastelander Ritualist"] = "Пустынник-ритуалист"
L["Wastes Creeper"] = "Ползун из пустошей"
L["Watcher Gashra"] = "Дозорный Гашра"
L["Watcher Irideus"] = "Страж Иридий"
L["Watcher Narjil"] = "Дозорный Нарджил"
L["Watcher Silthik"] = "Дозорный Силтик"
L["Watchful Inquisitor"] = "Бдительный инквизитор"
L["Waterlogged Soul Guard"] = "Промокший насквозь страж душ"
L["Waterworks Crocolisk"] = "Кроколиск с водокачки"
L["Waycrest Manor"] = "Усадьба Уэйкрестов"
L["Waycrest Reveler"] = "Гуляка из дома Уэйкрестов"
L["waycrestManorShortName"] = "УЭ"
L["wcmWorldquestNote"] =
  "Примечание к Г52:%sГ52 не будет присутствовать, пока активно локальное задание на победу над Матроной Кристиан."
L["Weald Shimmermoth"] = "Чащобный мерцающий мотылек"
L["Weaponized Crawler"] = "Вооруженный робопаук"
L["Weapons Tester"] = "Испытатель экспериментального оружия"
L["Web Marauder"] = "Паутинный мародер"
L["Welding Bot"] = "Сварочный робот"
L["Whisper of Fate"] = "Шепот судьбы"
L["Wholesome Hostess"] = "Благонравная горничная"
L["Wicked Oppressor"] = "Зловещая подавительница"
L["Wild Lasher"] = "Безумный плеточник"
L["Wild Vortex"] = "Дикое завихрение"
L["Wilted Oak"] = "Увядший дуб"
L["Windspeaker Heldis"] = "Говорящая с ветрами Хелдис"
L["Wing Leader Ner'onok"] = "Командующий флангом Нер'онок"
L["Winged Carrier"] = "Крылатый переносчик"
L["Winter Revenant"] = "Зимний загробник"
L["Winter Rumbler"] = "Зимний грохотун"
L["Wise Mari"] = "Мудрый Марис"
L["Wither Biter"] = "Иссохшая кусака"
L["Wither Slasher"] = "Рубака-иссушительница"
L["Witherbark"] = "Сухокожий"
L["Withered Fiend"] = "Иссохший злодей"
L["Withered Manawraith"] = "Иссохший - магический призрак"
L["Witherling"] = "Иссушительница"
L["Wo Relic"] = "Реликвия Во"
L["Worker Bee"] = "Рабочая пчела"
L["Workshop Defender"] = "Защитник мастерской"
L["Wrath of Azshara"] = "Гнев Азшары"
L["Wrath of the Lich King"] = "Гнев Короля-лича"
L["Wrathguard Bladelord"] = "Страж гнева - мастер клинка"
L["Wrathguard Felblade"] = "Страж гнева - клинок Скверны"
L["Wrathguard Flamebringer"] = "Страж гнева – возжигатель"
L["Wrathguard Invader"] = "Страж гнева - завоеватель"
L["Wretched Belcher"] = "Гнусный изрыгатель"
L["Wretched Plagueborer"] = "Жалкий чумной бурильщик"
L["Wriggling Darkspawn"] = "Извивающееся порождение Тьмы"
L["Wyrmtongue Scavenger"] = "Змееуст-барахольщик"
L["Wyrmtongue Trickster"] = "Змееуст-плутишка"
L["Xav the Unfallen"] = "Ксав Несломленный"
L["XB-488 Disposalbot"] = "Утилятор XB-488"
L["XD-175 Compactobot"] = "Компактобот XD-175"
L["Xephitik"] = "Ксеф'итик"
L["Xevozz"] = "Ксевозз"
L["Xiang"] = "Сян"
L["Xin the Weaponmaster"] = "Синь Мастер Боя"
L["Xira the Underhanded"] = "Зира Двурушница"
L["XR-949 Salvagebot"] = "Добычатор XR-949"
L["XT-002 Deconstructor"] = "Разрушитель XT-002"
L["Yalnu"] = "Йалну"
L["Yan-Zhu the Uncasked"] = "Янь-Чжу Высвобожденный"
L["Yazma"] = "Язма"
L["Yeasty Brew Alemental"] = "Пузыристый бражный хмелементаль"
L["Yes Man"] = "Поддакиватель"
L["Ymirjar Berserker"] = "Имирьяр-берсерк"
L["Ymirjar Dusk Shaman"] = "Имирьяр - шаман заката"
L["Ymirjar Flesh Hunter"] = "Имирьяр-плотоед"
L["Ymirjar Harpooner"] = "Имирьяр-гарпунщик"
L["Ymirjar Necromancer"] = "Имирьяр-некромант"
L["Ymirjar Savage"] = "Имирьяр-дикарь"
L["Ymirjar Warrior"] = "Имирьяр-воин"
L["Ymirjar Witch Doctor"] = "Имирьяр-знахарь"
L["Ymiron, the Fallen King"] = "Имирон, падший король"
L["Yogg-Saron"] = "Йогг-Сарон"
L["You are using MDT. You rock!"] = "Вы используете MDT. Вы крут!"
L["You can always return to the current Live Session preset by clicking the 'Return to Live' button next to the preset-dropdown"] =
  "Вы всегда можете вернуться к текущему маршруту в 'Прямой трансляции', нажав на кнопку 'Вернуться к Прямой трансляции' рядом с раскрывающимся списком маршрутов"
L["You can choose from different color palettes in the automatic pull coloring settings menu."] =
  "Вы можете выбрать различные цветовые палитры в меню настроек."
L["You can cycle through different floors by holding CTRL and using the mousewheel."] =
  "Вы можете переключаться между разными этажами, используя SHIFT + CTRL + колесо мыши."
L["You can cycle through dungeons by holding ALT and using the mousewheel."] =
  "Вы можете переключаться между подземельями, удерживая клавишу ALT и используя колесо мыши."
L["Young Storm Dragon"] = "Молодой грозовой дракон"
L["Yu'lon"] = "Юй-лун <Нефритовая Змея>"
L["Zanazal the Wise"] = "Заназал Мудрый"
L["Zanchuli Witch-Doctor"] = "Занчульская знахарка"
L["Zealous Templar"] = "Рьяная храмовница"
L["Zoggosh"] = "Зоггош"
L["Zo'gron"] = "Зо'грон"
L["Zolramus Bonecarver"] = "Резчик с Золрамуса"
L["Zolramus Bonemender"] = "Костоправ с Золрамуса"
L["Zolramus Gatekeeper"] = "Хранитель врат с Золрамуса"
L["Zolramus Necromancer"] = "Некромант с Золрамуса"
L["Zolramus Siphoner"] = "Похитительница с "
L["Zolramus Sorcerer"] = "Чародей с Золрамуса"
L["Zoom"] = "Масштаб"
L["zoomIn"] = "Ближе"
L["zoomOut"] = "Дальше"
L["Zo'phex"] = "Зо'фекс"
L["Zul'gamux"] = "Зул'гамуз"
L["Zul'Gurub"] = "Зул'Гуруб"
L["Zuraal the Ascended"] = "Зураал Перерожденный"
L["Zuramat the Obliterator"] = "Зурамат Уничтожитель"
