...
function DC:AddCollectedDataToEnemyTable(dungeonIndex, ignoreSpells, ignoreCC)
	...
	if not dungeonIndex then
		dungeonIndex = db.currentDungeonIdx
	end
	...
	if not ignoreSpells then
		print("Added " .. spellsAdded .. " new spells")
	end
	if not ignoreCC then
		print("Added " .. ccAdded .. " new CC characteristics")
	end
end
				...
				if enemy.id == id then
					for characteristic, data in pairs(characteristicsData) do
						...
						if spells and spells[spellId] then
							...
							if enemy.characteristics and enemy.characteristics[characteristic] then
								return
							end
							enemy.characteristics = enemy.characteristics or {}
							enemy.characteristics[characteristic] = true
		...
		if enemies then
			...
			for enemyIdx, enemy in pairs(enemies) do
				...
				if tracked then
					...
					local baseHealth = MDT:ReverseCalcEnemyHealth(
						tracked.health,
						tracked.level,
						isBoss,
						tracked.fortified,
